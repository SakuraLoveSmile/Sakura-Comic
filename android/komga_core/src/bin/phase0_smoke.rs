...
use komga_core::cache::cover::CoverStore;
use komga_core::cache::DiskCache;
...
async fn run(args: &Args) -> Result<()> {
    ...
    let cache_root = args
        ...
        .join("cache");
    let cache = DiskCache::new(&cache_root).map_err(|e| ApiError::Storage {
        message: e.to_string(),
    })?;
    let cover_store = CoverStore::new(cache, args.base_url.clone());
    let first = &rows[0];
    let cover_path = cover_store
        .ensure_thumbnail(&client, &args.server_id, &first.remote_id)
        .await?;
    println!("  cached {} -> {}", first.name, cover_path.display());
    Ok(())
}
...
async fn run_fixture(args: &Args) -> Result<()> {
    ...
    let first = &rows[0];
    let cover_path = cover_store
        .ensure_thumbnail(&FakeCoverFetcher, &args.server_id, &first.remote_id)
        .await?;
    println!("  cached {} -> {}", first.name, cover_path.display());
    Ok(())
}
