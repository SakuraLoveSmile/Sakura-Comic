// This file is automatically generated, so please do not edit it.
// @generated by `flutter_rust_bridge`@ 2.13.0.

// ignore_for_file: unused_import, unused_element, unnecessary_import, duplicate_ignore, invalid_use_of_internal_member, annotate_overrides, non_constant_identifier_names, curly_braces_in_flow_control_structures, prefer_const_literals_to_create_immutables, unused_field

import 'dart:async';
import 'dart:convert';
import 'diagnostics/log.dart';
import 'diagnostics/snapshot.dart';
import 'ffi/application.dart';
import 'ffi/bridge.dart';
import 'ffi/error.dart';
import 'frb_generated.dart';
import 'frb_generated.io.dart'
    if (dart.library.js_interop) 'frb_generated.web.dart';
import 'model/server.dart';
import 'model/server_profile.dart';
import 'package:flutter_rust_bridge/flutter_rust_bridge_for_generated.dart';
import 'store.dart';
import 'store/books.dart';
import 'store/collections.dart';
import 'store/prune.dart';
import 'store/query.dart';
import 'store/read_progress.dart';
import 'store/readlists.dart';
import 'store/series.dart';
import 'store/sync_state.dart';
import 'store/thumbnails.dart';
import 'sync/bootstrap.dart';
import 'sync/full.dart';
import 'sync/reconcile.dart';

/// Main entrypoint of the Rust API
class RustLib extends BaseEntrypoint<RustLibApi, RustLibApiImpl, RustLibWire> {
  @internal
  static final instance = RustLib._();

  RustLib._();

  /// Initialize flutter_rust_bridge
  static Future<void> init({
    RustLibApi? api,
    BaseHandler? handler,
    ExternalLibrary? externalLibrary,
    bool forceSameCodegenVersion = true,
  }) async {
    await instance.initImpl(
      api: api,
      handler: handler,
      externalLibrary: externalLibrary,
      forceSameCodegenVersion: forceSameCodegenVersion,
    );
  }

  /// Initialize flutter_rust_bridge in mock mode.
  /// No libraries for FFI are loaded.
  static void initMock({
    required RustLibApi api,
  }) {
    instance.initMockImpl(
      api: api,
    );
  }

  /// Dispose flutter_rust_bridge
  ///
  /// The call to this function is optional, since flutter_rust_bridge (and everything else)
  /// is automatically disposed when the app stops.
  static void dispose() => instance.disposeImpl();

  @override
  ApiImplConstructor<RustLibApiImpl, RustLibWire> get apiImplConstructor =>
      RustLibApiImpl.new;

  @override
  WireConstructor<RustLibWire> get wireConstructor =>
      RustLibWire.fromExternalLibrary;

  @override
  Future<void> executeRustInitializers() async {}

  @override
  ExternalLibraryLoaderConfig get defaultExternalLibraryLoaderConfig =>
      kDefaultExternalLibraryLoaderConfig;

  @override
  String get codegenVersion => '2.13.0';

  @override
  int get rustContentHash => -1134374146;

  static const kDefaultExternalLibraryLoaderConfig =
      ExternalLibraryLoaderConfig(
    stem: 'komga_core',
    ioDirectory: '../komga_core/target/release/',
    webPrefix: 'pkg/',
    wasmBindgenName: 'wasm_bindgen',
  );
}

abstract class RustLibApi extends BaseApi {
  Future<AuthStateDto> crateFfiBridgeAuthState(
      {required String dbPath, required String serverId});

  Future<String?> crateFfiBridgeBookCoverPath(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<BookDetailRow?> crateFfiBridgeBookDetail(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<BootstrapSummary> crateFfiBridgeBootstrap(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey});

  Future<BootstrapSummary> crateFfiBridgeBootstrapDemo(
      {required String dbPath, required String serverId});

  Future<FullSyncSummary> crateFfiBridgeBootstrapSync(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required bool resume});

  Future<CollectionDetailRow?> crateFfiBridgeCollectionDetail(
      {required String dbPath,
      required String serverId,
      required String collectionId,
      required PlatformInt64 limit,
      required PlatformInt64 offset});

  Future<List<ContinueReadingRow>> crateFfiBridgeContinueReading(
      {required String dbPath,
      required String serverId,
      required PlatformInt64 limit});

  Future<String?> crateFfiBridgeCoverPath(
      {required String dbPath,
      required String serverId,
      required String seriesId});

  Future<Map<String, String>> crateFfiBridgeCoverPaths(
      {required String dbPath,
      required String serverId,
      required String variant,
      required List<String> remoteIds});

  Future<bool> crateFfiBridgeDeleteServer(
      {required String dbPath, required String serverId});

  Future<LogStats> crateFfiBridgeDiagnosticsLogStats();

  Future<List<LogRecord>> crateFfiBridgeDiagnosticsLogs(
      {required PlatformInt64 limit, required String minLevel});

  Future<DiagnosticsDto> crateFfiBridgeDiagnosticsSnapshot(
      {required String dbPath, required String serverId});

  Future<DownloadDeleteDto> crateFfiBridgeDownloadDelete(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<DownloadDeleteDto> crateFfiBridgeDownloadDeleteAll(
      {required String dbPath, required String serverId});

  Future<DownloadBookDto> crateFfiBridgeDownloadEnqueue(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<List<DownloadBookDto>> crateFfiBridgeDownloadList(
      {required String dbPath, required String serverId});

  Future<DownloadBookDto> crateFfiBridgeDownloadPause(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<DownloadPumpDto?> crateFfiBridgeDownloadPump(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required PlatformInt64 maxPages,
      required PlatformInt64 maxBytes,
      required PlatformInt64 freeVolumeBytes,
      required String link});

  Future<DownloadBookDto> crateFfiBridgeDownloadResume(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<DownloadBookDto> crateFfiBridgeDownloadRetry(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<DownloadBookDto> crateFfiBridgeDownloadSetAllowCellular(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required bool allow});

  Future<StorageDto> crateFfiBridgeDownloadStorage(
      {required String dbPath, required PlatformInt64 freeVolumeBytes});

  Future<DownloadSweepDto> crateFfiBridgeDownloadSweep(
      {required String dbPath});

  Future<String> crateFfiBridgeEnsureBookCover(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required String baseUrl,
      required String apiKey});

  Future<PlatformInt64> crateFfiBridgeEnsureBookCovers(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      required String baseUrl,
      required String apiKey});

  Future<String> crateFfiBridgeEnsureCover(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      required String baseUrl,
      required String apiKey});

  Future<PlatformInt64> crateFfiBridgeEnsureCovers(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey});

  Future<List<SeriesRow>> crateFfiBridgeFetchSeries(
      {required String dbPath,
      required String serverId,
      required PlatformInt64 limit,
      required PlatformInt64 offset});

  Future<FilterOptions> crateFfiBridgeFilterOptions(
      {required String dbPath, required String serverId});

  Future<FullSyncSummary> crateFfiBridgeFullSync(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey});

  Future<String?> crateFfiBridgeGetActiveServer({required String dbPath});

  Future<ServerProfile?> crateFfiBridgeGetServer(
      {required String dbPath, required String serverId});

  Future<List<LibraryCountRow>> crateFfiBridgeLibraryCounts(
      {required String dbPath, required String serverId});

  Future<LibraryCountRow?> crateFfiBridgeLibraryDetail(
      {required String dbPath,
      required String serverId,
      required String libraryId});

  Future<CollectionPageResult> crateFfiBridgeListCollections(
      {required String dbPath,
      required String serverId,
      String? search,
      required PlatformInt64 limit,
      required PlatformInt64 offset});

  Future<ReadlistPageResult> crateFfiBridgeListReadlists(
      {required String dbPath,
      required String serverId,
      String? search,
      required PlatformInt64 limit,
      required PlatformInt64 offset});

  Future<List<ServerProfile>> crateFfiBridgeListServers(
      {required String dbPath});

  Future<List<ThumbnailRow>> crateFfiBridgeListThumbnails(
      {required String dbPath, required String serverId});

  Future<void> crateFfiBridgeMarkRead(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<void> crateFfiBridgeMarkUnread(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<OutboxStatusDto> crateFfiBridgeOutboxStatus(
      {required String dbPath, required String serverId});

  Future<BookPageResult> crateFfiBridgeQueryBooks(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      String? search,
      String? readStatus,
      String? tag,
      required String sort,
      required bool ascending,
      required PlatformInt64 limit,
      required PlatformInt64 offset});

  Future<SeriesPageResult> crateFfiBridgeQuerySeries(
      {required String dbPath,
      required String serverId,
      String? search,
      String? libraryId,
      String? status,
      String? tag,
      String? genre,
      required String sort,
      required bool ascending,
      required PlatformInt64 limit,
      required PlatformInt64 offset});

  Future<bool> crateFfiBridgeReaderBackground(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<CacheStatsDto> crateFfiBridgeReaderCacheStats(
      {required String dbPath});

  Future<PlatformInt64> crateFfiBridgeReaderClearPrefetch(
      {required String dbPath});

  Future<bool> crateFfiBridgeReaderClose(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<ReaderWindowDto> crateFfiBridgeReaderConfigureDevice(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required DeviceProfileDto device});

  Future<bool> crateFfiBridgeReaderMarkRead(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<bool> crateFfiBridgeReaderMarkUnread(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<ReaderBookDto> crateFfiBridgeReaderOpen(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required String baseUrl,
      required String apiKey,
      required String mode,
      required String direction,
      bool? firstPageSingle});

  Future<String> crateFfiBridgeReaderPage(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page,
      required String baseUrl,
      required String apiKey});

  Future<String?> crateFfiBridgeReaderPagePath(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page});

  Future<PlatformInt64> crateFfiBridgeReaderPrefetch(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 spread,
      required String baseUrl,
      required String apiKey});

  Future<CacheCleanupDto> crateFfiBridgeReaderReconcileCache(
      {required String dbPath});

  Future<PlatformInt64> crateFfiBridgeReaderReleasePrefetch(
      {required String dbPath});

  Future<ReaderLayoutDto> crateFfiBridgeReaderSetLayout(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required String mode,
      required String direction});

  Future<void> crateFfiBridgeReaderSetPageOffset(
      {required String dbPath,
      required String serverId,
      required String bookId,
      double? ratio});

  Future<ReaderSettingsDto> crateFfiBridgeReaderSetSettings(
      {required String dbPath, required ReaderSettingsDto settings});

  Future<ReaderSettingsDto> crateFfiBridgeReaderSettings(
      {required String dbPath});

  Future<ReaderTurnDto> crateFfiBridgeReaderStep(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 delta});

  Future<bool> crateFfiBridgeReaderTick(
      {required String dbPath,
      required String serverId,
      required String bookId});

  Future<ReaderTurnDto> crateFfiBridgeReaderTurn(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page});

  Future<ReadlistDetailRow?> crateFfiBridgeReadlistDetail(
      {required String dbPath,
      required String serverId,
      required String readlistId,
      required PlatformInt64 limit,
      required PlatformInt64 offset});

  Future<ReconcileSummary> crateFfiBridgeReconcile(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required String trigger});

  Future<PlatformInt64> crateFfiBridgeRetryFailedMutations(
      {required String dbPath, required String serverId});

  Future<void> crateFfiBridgeSaveLibraries(
      {required String dbPath,
      required String serverId,
      required List<Library> libraries});

  Future<void> crateFfiBridgeSaveServer(
      {required String dbPath, required ServerProfile profile});

  Future<SeriesDetailRow?> crateFfiBridgeSeriesDetail(
      {required String dbPath,
      required String serverId,
      required String seriesId});

  Future<String?> crateFfiBridgeSeriesReadOverride(
      {required String dbPath,
      required String serverId,
      required String seriesId});

  Future<ReadTargetRow> crateFfiBridgeSeriesReadTarget(
      {required String dbPath,
      required String serverId,
      required String seriesId});

  Future<void> crateFfiBridgeSetActiveServer(
      {required String dbPath, required String serverId});

  Future<void> crateFfiBridgeSetReadProgress(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page,
      required bool completed});

  Future<String?> crateFfiBridgeSetSeriesReadOverride(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      String? mode,
      String? direction});

  Future<bool> crateFfiBridgeShouldReconcile(
      {required String dbPath,
      required String serverId,
      required String trigger});

  Future<SsePollResult?> crateFfiBridgeSsePoll(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required String stateJson});

  Future<String> crateFfiBridgeSseReconciled(
      {required String dbPath, required String stateJson});

  Future<String> crateFfiBridgeSseResume(
      {required String dbPath, required String stateJson});

  Future<void> crateFfiBridgeSseStop(
      {required String dbPath, required String serverId});

  Future<List<EntitySyncState>> crateFfiBridgeSyncStates(
      {required String dbPath, required String serverId});

  Future<ConnectionResult> crateFfiBridgeTestConnection(
      {required String baseUrl, required String apiKey});

  Future<List<Tombstone>> crateFfiBridgeTombstones(
      {required String dbPath,
      required String serverId,
      required String entityType});

  Future<UploadOutcomeDto> crateFfiBridgeUploadOutbox(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey});
}

class RustLibApiImpl extends RustLibApiImplPlatform implements RustLibApi {
  RustLibApiImpl({
    required super.handler,
    required super.wire,
    required super.generalizedFrbRustBinding,
    required super.portManager,
  });

  @override
  Future<AuthStateDto> crateFfiBridgeAuthState(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 1, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_auth_state_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeAuthStateConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeAuthStateConstMeta => const TaskConstMeta(
        debugName: "auth_state",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<String?> crateFfiBridgeBookCoverPath(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 2, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeBookCoverPathConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeBookCoverPathConstMeta =>
      const TaskConstMeta(
        debugName: "book_cover_path",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<BookDetailRow?> crateFfiBridgeBookDetail(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 3, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_book_detail_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeBookDetailConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeBookDetailConstMeta => const TaskConstMeta(
        debugName: "book_detail",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<BootstrapSummary> crateFfiBridgeBootstrap(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 4, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bootstrap_summary,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeBootstrapConstMeta,
      argValues: [dbPath, serverId, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeBootstrapConstMeta => const TaskConstMeta(
        debugName: "bootstrap",
        argNames: ["dbPath", "serverId", "baseUrl", "apiKey"],
      );

  @override
  Future<BootstrapSummary> crateFfiBridgeBootstrapDemo(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 5, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bootstrap_summary,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeBootstrapDemoConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeBootstrapDemoConstMeta =>
      const TaskConstMeta(
        debugName: "bootstrap_demo",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<FullSyncSummary> crateFfiBridgeBootstrapSync(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required bool resume}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        sse_encode_bool(resume, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 6, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_full_sync_summary,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeBootstrapSyncConstMeta,
      argValues: [dbPath, serverId, baseUrl, apiKey, resume],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeBootstrapSyncConstMeta =>
      const TaskConstMeta(
        debugName: "bootstrap_sync",
        argNames: ["dbPath", "serverId", "baseUrl", "apiKey", "resume"],
      );

  @override
  Future<CollectionDetailRow?> crateFfiBridgeCollectionDetail(
      {required String dbPath,
      required String serverId,
      required String collectionId,
      required PlatformInt64 limit,
      required PlatformInt64 offset}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(collectionId, serializer);
        sse_encode_i_64(limit, serializer);
        sse_encode_i_64(offset, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 7, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_collection_detail_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeCollectionDetailConstMeta,
      argValues: [dbPath, serverId, collectionId, limit, offset],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeCollectionDetailConstMeta =>
      const TaskConstMeta(
        debugName: "collection_detail",
        argNames: ["dbPath", "serverId", "collectionId", "limit", "offset"],
      );

  @override
  Future<List<ContinueReadingRow>> crateFfiBridgeContinueReading(
      {required String dbPath,
      required String serverId,
      required PlatformInt64 limit}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_i_64(limit, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 8, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_continue_reading_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeContinueReadingConstMeta,
      argValues: [dbPath, serverId, limit],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeContinueReadingConstMeta =>
      const TaskConstMeta(
        debugName: "continue_reading",
        argNames: ["dbPath", "serverId", "limit"],
      );

  @override
  Future<String?> crateFfiBridgeCoverPath(
      {required String dbPath,
      required String serverId,
      required String seriesId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 9, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeCoverPathConstMeta,
      argValues: [dbPath, serverId, seriesId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeCoverPathConstMeta => const TaskConstMeta(
        debugName: "cover_path",
        argNames: ["dbPath", "serverId", "seriesId"],
      );

  @override
  Future<Map<String, String>> crateFfiBridgeCoverPaths(
      {required String dbPath,
      required String serverId,
      required String variant,
      required List<String> remoteIds}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(variant, serializer);
        sse_encode_list_String(remoteIds, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 10, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_Map_String_String_None,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeCoverPathsConstMeta,
      argValues: [dbPath, serverId, variant, remoteIds],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeCoverPathsConstMeta => const TaskConstMeta(
        debugName: "cover_paths",
        argNames: ["dbPath", "serverId", "variant", "remoteIds"],
      );

  @override
  Future<bool> crateFfiBridgeDeleteServer(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 11, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bool,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDeleteServerConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDeleteServerConstMeta => const TaskConstMeta(
        debugName: "delete_server",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<LogStats> crateFfiBridgeDiagnosticsLogStats() {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 12, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_log_stats,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDiagnosticsLogStatsConstMeta,
      argValues: [],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDiagnosticsLogStatsConstMeta =>
      const TaskConstMeta(
        debugName: "diagnostics_log_stats",
        argNames: [],
      );

  @override
  Future<List<LogRecord>> crateFfiBridgeDiagnosticsLogs(
      {required PlatformInt64 limit, required String minLevel}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_i_64(limit, serializer);
        sse_encode_String(minLevel, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 13, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_log_record,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDiagnosticsLogsConstMeta,
      argValues: [limit, minLevel],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDiagnosticsLogsConstMeta =>
      const TaskConstMeta(
        debugName: "diagnostics_logs",
        argNames: ["limit", "minLevel"],
      );

  @override
  Future<DiagnosticsDto> crateFfiBridgeDiagnosticsSnapshot(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 14, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_diagnostics_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDiagnosticsSnapshotConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDiagnosticsSnapshotConstMeta =>
      const TaskConstMeta(
        debugName: "diagnostics_snapshot",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<DownloadDeleteDto> crateFfiBridgeDownloadDelete(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 15, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_delete_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadDeleteConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadDeleteConstMeta =>
      const TaskConstMeta(
        debugName: "download_delete",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<DownloadDeleteDto> crateFfiBridgeDownloadDeleteAll(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 16, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_delete_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadDeleteAllConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadDeleteAllConstMeta =>
      const TaskConstMeta(
        debugName: "download_delete_all",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<DownloadBookDto> crateFfiBridgeDownloadEnqueue(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 17, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_book_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadEnqueueConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadEnqueueConstMeta =>
      const TaskConstMeta(
        debugName: "download_enqueue",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<List<DownloadBookDto>> crateFfiBridgeDownloadList(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 18, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_download_book_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadListConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadListConstMeta => const TaskConstMeta(
        debugName: "download_list",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<DownloadBookDto> crateFfiBridgeDownloadPause(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 19, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_book_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadPauseConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadPauseConstMeta =>
      const TaskConstMeta(
        debugName: "download_pause",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<DownloadPumpDto?> crateFfiBridgeDownloadPump(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required PlatformInt64 maxPages,
      required PlatformInt64 maxBytes,
      required PlatformInt64 freeVolumeBytes,
      required String link}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        sse_encode_i_64(maxPages, serializer);
        sse_encode_i_64(maxBytes, serializer);
        sse_encode_i_64(freeVolumeBytes, serializer);
        sse_encode_String(link, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 20, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_download_pump_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadPumpConstMeta,
      argValues: [
        dbPath,
        serverId,
        baseUrl,
        apiKey,
        maxPages,
        maxBytes,
        freeVolumeBytes,
        link
      ],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadPumpConstMeta => const TaskConstMeta(
        debugName: "download_pump",
        argNames: [
          "dbPath",
          "serverId",
          "baseUrl",
          "apiKey",
          "maxPages",
          "maxBytes",
          "freeVolumeBytes",
          "link"
        ],
      );

  @override
  Future<DownloadBookDto> crateFfiBridgeDownloadResume(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 21, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_book_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadResumeConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadResumeConstMeta =>
      const TaskConstMeta(
        debugName: "download_resume",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<DownloadBookDto> crateFfiBridgeDownloadRetry(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 22, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_book_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadRetryConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadRetryConstMeta =>
      const TaskConstMeta(
        debugName: "download_retry",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<DownloadBookDto> crateFfiBridgeDownloadSetAllowCellular(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required bool allow}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_bool(allow, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 23, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_book_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadSetAllowCellularConstMeta,
      argValues: [dbPath, serverId, bookId, allow],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadSetAllowCellularConstMeta =>
      const TaskConstMeta(
        debugName: "download_set_allow_cellular",
        argNames: ["dbPath", "serverId", "bookId", "allow"],
      );

  @override
  Future<StorageDto> crateFfiBridgeDownloadStorage(
      {required String dbPath, required PlatformInt64 freeVolumeBytes}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_i_64(freeVolumeBytes, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 24, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_storage_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadStorageConstMeta,
      argValues: [dbPath, freeVolumeBytes],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadStorageConstMeta =>
      const TaskConstMeta(
        debugName: "download_storage",
        argNames: ["dbPath", "freeVolumeBytes"],
      );

  @override
  Future<DownloadSweepDto> crateFfiBridgeDownloadSweep(
      {required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 25, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_download_sweep_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeDownloadSweepConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeDownloadSweepConstMeta =>
      const TaskConstMeta(
        debugName: "download_sweep",
        argNames: ["dbPath"],
      );

  @override
  Future<String> crateFfiBridgeEnsureBookCover(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 26, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeEnsureBookCoverConstMeta,
      argValues: [dbPath, serverId, bookId, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeEnsureBookCoverConstMeta =>
      const TaskConstMeta(
        debugName: "ensure_book_cover",
        argNames: ["dbPath", "serverId", "bookId", "baseUrl", "apiKey"],
      );

  @override
  Future<PlatformInt64> crateFfiBridgeEnsureBookCovers(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 27, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_i_64,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeEnsureBookCoversConstMeta,
      argValues: [dbPath, serverId, seriesId, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeEnsureBookCoversConstMeta =>
      const TaskConstMeta(
        debugName: "ensure_book_covers",
        argNames: ["dbPath", "serverId", "seriesId", "baseUrl", "apiKey"],
      );

  @override
  Future<String> crateFfiBridgeEnsureCover(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 28, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeEnsureCoverConstMeta,
      argValues: [dbPath, serverId, seriesId, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeEnsureCoverConstMeta => const TaskConstMeta(
        debugName: "ensure_cover",
        argNames: ["dbPath", "serverId", "seriesId", "baseUrl", "apiKey"],
      );

  @override
  Future<PlatformInt64> crateFfiBridgeEnsureCovers(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 29, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_i_64,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeEnsureCoversConstMeta,
      argValues: [dbPath, serverId, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeEnsureCoversConstMeta => const TaskConstMeta(
        debugName: "ensure_covers",
        argNames: ["dbPath", "serverId", "baseUrl", "apiKey"],
      );

  @override
  Future<List<SeriesRow>> crateFfiBridgeFetchSeries(
      {required String dbPath,
      required String serverId,
      required PlatformInt64 limit,
      required PlatformInt64 offset}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_i_64(limit, serializer);
        sse_encode_i_64(offset, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 30, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_series_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeFetchSeriesConstMeta,
      argValues: [dbPath, serverId, limit, offset],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeFetchSeriesConstMeta => const TaskConstMeta(
        debugName: "fetch_series",
        argNames: ["dbPath", "serverId", "limit", "offset"],
      );

  @override
  Future<FilterOptions> crateFfiBridgeFilterOptions(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 31, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_filter_options,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeFilterOptionsConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeFilterOptionsConstMeta =>
      const TaskConstMeta(
        debugName: "filter_options",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<FullSyncSummary> crateFfiBridgeFullSync(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 32, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_full_sync_summary,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeFullSyncConstMeta,
      argValues: [dbPath, serverId, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeFullSyncConstMeta => const TaskConstMeta(
        debugName: "full_sync",
        argNames: ["dbPath", "serverId", "baseUrl", "apiKey"],
      );

  @override
  Future<String?> crateFfiBridgeGetActiveServer({required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 33, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeGetActiveServerConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeGetActiveServerConstMeta =>
      const TaskConstMeta(
        debugName: "get_active_server",
        argNames: ["dbPath"],
      );

  @override
  Future<ServerProfile?> crateFfiBridgeGetServer(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 34, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_server_profile,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeGetServerConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeGetServerConstMeta => const TaskConstMeta(
        debugName: "get_server",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<List<LibraryCountRow>> crateFfiBridgeLibraryCounts(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 35, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_library_count_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeLibraryCountsConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeLibraryCountsConstMeta =>
      const TaskConstMeta(
        debugName: "library_counts",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<LibraryCountRow?> crateFfiBridgeLibraryDetail(
      {required String dbPath,
      required String serverId,
      required String libraryId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(libraryId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 36, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_library_count_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeLibraryDetailConstMeta,
      argValues: [dbPath, serverId, libraryId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeLibraryDetailConstMeta =>
      const TaskConstMeta(
        debugName: "library_detail",
        argNames: ["dbPath", "serverId", "libraryId"],
      );

  @override
  Future<CollectionPageResult> crateFfiBridgeListCollections(
      {required String dbPath,
      required String serverId,
      String? search,
      required PlatformInt64 limit,
      required PlatformInt64 offset}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_opt_String(search, serializer);
        sse_encode_i_64(limit, serializer);
        sse_encode_i_64(offset, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 37, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_collection_page_result,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeListCollectionsConstMeta,
      argValues: [dbPath, serverId, search, limit, offset],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeListCollectionsConstMeta =>
      const TaskConstMeta(
        debugName: "list_collections",
        argNames: ["dbPath", "serverId", "search", "limit", "offset"],
      );

  @override
  Future<ReadlistPageResult> crateFfiBridgeListReadlists(
      {required String dbPath,
      required String serverId,
      String? search,
      required PlatformInt64 limit,
      required PlatformInt64 offset}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_opt_String(search, serializer);
        sse_encode_i_64(limit, serializer);
        sse_encode_i_64(offset, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 38, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_readlist_page_result,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeListReadlistsConstMeta,
      argValues: [dbPath, serverId, search, limit, offset],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeListReadlistsConstMeta =>
      const TaskConstMeta(
        debugName: "list_readlists",
        argNames: ["dbPath", "serverId", "search", "limit", "offset"],
      );

  @override
  Future<List<ServerProfile>> crateFfiBridgeListServers(
      {required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 39, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_server_profile,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeListServersConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeListServersConstMeta => const TaskConstMeta(
        debugName: "list_servers",
        argNames: ["dbPath"],
      );

  @override
  Future<List<ThumbnailRow>> crateFfiBridgeListThumbnails(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 40, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_thumbnail_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeListThumbnailsConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeListThumbnailsConstMeta =>
      const TaskConstMeta(
        debugName: "list_thumbnails",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<void> crateFfiBridgeMarkRead(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 41, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeMarkReadConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeMarkReadConstMeta => const TaskConstMeta(
        debugName: "mark_read",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<void> crateFfiBridgeMarkUnread(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 42, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeMarkUnreadConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeMarkUnreadConstMeta => const TaskConstMeta(
        debugName: "mark_unread",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<OutboxStatusDto> crateFfiBridgeOutboxStatus(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 43, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_outbox_status_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeOutboxStatusConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeOutboxStatusConstMeta => const TaskConstMeta(
        debugName: "outbox_status",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<BookPageResult> crateFfiBridgeQueryBooks(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      String? search,
      String? readStatus,
      String? tag,
      required String sort,
      required bool ascending,
      required PlatformInt64 limit,
      required PlatformInt64 offset}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        sse_encode_opt_String(search, serializer);
        sse_encode_opt_String(readStatus, serializer);
        sse_encode_opt_String(tag, serializer);
        sse_encode_String(sort, serializer);
        sse_encode_bool(ascending, serializer);
        sse_encode_i_64(limit, serializer);
        sse_encode_i_64(offset, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 44, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_book_page_result,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeQueryBooksConstMeta,
      argValues: [
        dbPath,
        serverId,
        seriesId,
        search,
        readStatus,
        tag,
        sort,
        ascending,
        limit,
        offset
      ],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeQueryBooksConstMeta => const TaskConstMeta(
        debugName: "query_books",
        argNames: [
          "dbPath",
          "serverId",
          "seriesId",
          "search",
          "readStatus",
          "tag",
          "sort",
          "ascending",
          "limit",
          "offset"
        ],
      );

  @override
  Future<SeriesPageResult> crateFfiBridgeQuerySeries(
      {required String dbPath,
      required String serverId,
      String? search,
      String? libraryId,
      String? status,
      String? tag,
      String? genre,
      required String sort,
      required bool ascending,
      required PlatformInt64 limit,
      required PlatformInt64 offset}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_opt_String(search, serializer);
        sse_encode_opt_String(libraryId, serializer);
        sse_encode_opt_String(status, serializer);
        sse_encode_opt_String(tag, serializer);
        sse_encode_opt_String(genre, serializer);
        sse_encode_String(sort, serializer);
        sse_encode_bool(ascending, serializer);
        sse_encode_i_64(limit, serializer);
        sse_encode_i_64(offset, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 45, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_series_page_result,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeQuerySeriesConstMeta,
      argValues: [
        dbPath,
        serverId,
        search,
        libraryId,
        status,
        tag,
        genre,
        sort,
        ascending,
        limit,
        offset
      ],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeQuerySeriesConstMeta => const TaskConstMeta(
        debugName: "query_series",
        argNames: [
          "dbPath",
          "serverId",
          "search",
          "libraryId",
          "status",
          "tag",
          "genre",
          "sort",
          "ascending",
          "limit",
          "offset"
        ],
      );

  @override
  Future<bool> crateFfiBridgeReaderBackground(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 46, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bool,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderBackgroundConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderBackgroundConstMeta =>
      const TaskConstMeta(
        debugName: "reader_background",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<CacheStatsDto> crateFfiBridgeReaderCacheStats(
      {required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 47, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_cache_stats_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderCacheStatsConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderCacheStatsConstMeta =>
      const TaskConstMeta(
        debugName: "reader_cache_stats",
        argNames: ["dbPath"],
      );

  @override
  Future<PlatformInt64> crateFfiBridgeReaderClearPrefetch(
      {required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 48, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_i_64,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderClearPrefetchConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderClearPrefetchConstMeta =>
      const TaskConstMeta(
        debugName: "reader_clear_prefetch",
        argNames: ["dbPath"],
      );

  @override
  Future<bool> crateFfiBridgeReaderClose(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 49, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bool,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderCloseConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderCloseConstMeta => const TaskConstMeta(
        debugName: "reader_close",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<ReaderWindowDto> crateFfiBridgeReaderConfigureDevice(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required DeviceProfileDto device}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_box_autoadd_device_profile_dto(device, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 50, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reader_window_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderConfigureDeviceConstMeta,
      argValues: [dbPath, serverId, bookId, device],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderConfigureDeviceConstMeta =>
      const TaskConstMeta(
        debugName: "reader_configure_device",
        argNames: ["dbPath", "serverId", "bookId", "device"],
      );

  @override
  Future<bool> crateFfiBridgeReaderMarkRead(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 51, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bool,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderMarkReadConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderMarkReadConstMeta =>
      const TaskConstMeta(
        debugName: "reader_mark_read",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<bool> crateFfiBridgeReaderMarkUnread(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 52, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bool,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderMarkUnreadConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderMarkUnreadConstMeta =>
      const TaskConstMeta(
        debugName: "reader_mark_unread",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<ReaderBookDto> crateFfiBridgeReaderOpen(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required String baseUrl,
      required String apiKey,
      required String mode,
      required String direction,
      bool? firstPageSingle}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        sse_encode_String(mode, serializer);
        sse_encode_String(direction, serializer);
        sse_encode_opt_box_autoadd_bool(firstPageSingle, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 53, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reader_book_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderOpenConstMeta,
      argValues: [
        dbPath,
        serverId,
        bookId,
        baseUrl,
        apiKey,
        mode,
        direction,
        firstPageSingle
      ],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderOpenConstMeta => const TaskConstMeta(
        debugName: "reader_open",
        argNames: [
          "dbPath",
          "serverId",
          "bookId",
          "baseUrl",
          "apiKey",
          "mode",
          "direction",
          "firstPageSingle"
        ],
      );

  @override
  Future<String> crateFfiBridgeReaderPage(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_i_64(page, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 54, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderPageConstMeta,
      argValues: [dbPath, serverId, bookId, page, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderPageConstMeta => const TaskConstMeta(
        debugName: "reader_page",
        argNames: ["dbPath", "serverId", "bookId", "page", "baseUrl", "apiKey"],
      );

  @override
  Future<String?> crateFfiBridgeReaderPagePath(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_i_64(page, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 55, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderPagePathConstMeta,
      argValues: [dbPath, serverId, bookId, page],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderPagePathConstMeta =>
      const TaskConstMeta(
        debugName: "reader_page_path",
        argNames: ["dbPath", "serverId", "bookId", "page"],
      );

  @override
  Future<PlatformInt64> crateFfiBridgeReaderPrefetch(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 spread,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_i_64(spread, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 56, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_i_64,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderPrefetchConstMeta,
      argValues: [dbPath, serverId, bookId, spread, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderPrefetchConstMeta =>
      const TaskConstMeta(
        debugName: "reader_prefetch",
        argNames: [
          "dbPath",
          "serverId",
          "bookId",
          "spread",
          "baseUrl",
          "apiKey"
        ],
      );

  @override
  Future<CacheCleanupDto> crateFfiBridgeReaderReconcileCache(
      {required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 57, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_cache_cleanup_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderReconcileCacheConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderReconcileCacheConstMeta =>
      const TaskConstMeta(
        debugName: "reader_reconcile_cache",
        argNames: ["dbPath"],
      );

  @override
  Future<PlatformInt64> crateFfiBridgeReaderReleasePrefetch(
      {required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 58, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_i_64,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderReleasePrefetchConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderReleasePrefetchConstMeta =>
      const TaskConstMeta(
        debugName: "reader_release_prefetch",
        argNames: ["dbPath"],
      );

  @override
  Future<ReaderLayoutDto> crateFfiBridgeReaderSetLayout(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required String mode,
      required String direction}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_String(mode, serializer);
        sse_encode_String(direction, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 59, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reader_layout_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderSetLayoutConstMeta,
      argValues: [dbPath, serverId, bookId, mode, direction],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderSetLayoutConstMeta =>
      const TaskConstMeta(
        debugName: "reader_set_layout",
        argNames: ["dbPath", "serverId", "bookId", "mode", "direction"],
      );

  @override
  Future<void> crateFfiBridgeReaderSetPageOffset(
      {required String dbPath,
      required String serverId,
      required String bookId,
      double? ratio}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_opt_box_autoadd_f_64(ratio, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 60, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderSetPageOffsetConstMeta,
      argValues: [dbPath, serverId, bookId, ratio],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderSetPageOffsetConstMeta =>
      const TaskConstMeta(
        debugName: "reader_set_page_offset",
        argNames: ["dbPath", "serverId", "bookId", "ratio"],
      );

  @override
  Future<ReaderSettingsDto> crateFfiBridgeReaderSetSettings(
      {required String dbPath, required ReaderSettingsDto settings}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_box_autoadd_reader_settings_dto(settings, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 61, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reader_settings_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderSetSettingsConstMeta,
      argValues: [dbPath, settings],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderSetSettingsConstMeta =>
      const TaskConstMeta(
        debugName: "reader_set_settings",
        argNames: ["dbPath", "settings"],
      );

  @override
  Future<ReaderSettingsDto> crateFfiBridgeReaderSettings(
      {required String dbPath}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 62, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reader_settings_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderSettingsConstMeta,
      argValues: [dbPath],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderSettingsConstMeta =>
      const TaskConstMeta(
        debugName: "reader_settings",
        argNames: ["dbPath"],
      );

  @override
  Future<ReaderTurnDto> crateFfiBridgeReaderStep(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 delta}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_i_64(delta, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 63, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reader_turn_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderStepConstMeta,
      argValues: [dbPath, serverId, bookId, delta],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderStepConstMeta => const TaskConstMeta(
        debugName: "reader_step",
        argNames: ["dbPath", "serverId", "bookId", "delta"],
      );

  @override
  Future<bool> crateFfiBridgeReaderTick(
      {required String dbPath,
      required String serverId,
      required String bookId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 64, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bool,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderTickConstMeta,
      argValues: [dbPath, serverId, bookId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderTickConstMeta => const TaskConstMeta(
        debugName: "reader_tick",
        argNames: ["dbPath", "serverId", "bookId"],
      );

  @override
  Future<ReaderTurnDto> crateFfiBridgeReaderTurn(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_i_64(page, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 65, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reader_turn_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReaderTurnConstMeta,
      argValues: [dbPath, serverId, bookId, page],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReaderTurnConstMeta => const TaskConstMeta(
        debugName: "reader_turn",
        argNames: ["dbPath", "serverId", "bookId", "page"],
      );

  @override
  Future<ReadlistDetailRow?> crateFfiBridgeReadlistDetail(
      {required String dbPath,
      required String serverId,
      required String readlistId,
      required PlatformInt64 limit,
      required PlatformInt64 offset}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(readlistId, serializer);
        sse_encode_i_64(limit, serializer);
        sse_encode_i_64(offset, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 66, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_readlist_detail_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReadlistDetailConstMeta,
      argValues: [dbPath, serverId, readlistId, limit, offset],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReadlistDetailConstMeta =>
      const TaskConstMeta(
        debugName: "readlist_detail",
        argNames: ["dbPath", "serverId", "readlistId", "limit", "offset"],
      );

  @override
  Future<ReconcileSummary> crateFfiBridgeReconcile(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required String trigger}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        sse_encode_String(trigger, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 67, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_reconcile_summary,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeReconcileConstMeta,
      argValues: [dbPath, serverId, baseUrl, apiKey, trigger],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeReconcileConstMeta => const TaskConstMeta(
        debugName: "reconcile",
        argNames: ["dbPath", "serverId", "baseUrl", "apiKey", "trigger"],
      );

  @override
  Future<PlatformInt64> crateFfiBridgeRetryFailedMutations(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 68, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_i_64,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeRetryFailedMutationsConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeRetryFailedMutationsConstMeta =>
      const TaskConstMeta(
        debugName: "retry_failed_mutations",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<void> crateFfiBridgeSaveLibraries(
      {required String dbPath,
      required String serverId,
      required List<Library> libraries}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_list_library(libraries, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 69, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSaveLibrariesConstMeta,
      argValues: [dbPath, serverId, libraries],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSaveLibrariesConstMeta =>
      const TaskConstMeta(
        debugName: "save_libraries",
        argNames: ["dbPath", "serverId", "libraries"],
      );

  @override
  Future<void> crateFfiBridgeSaveServer(
      {required String dbPath, required ServerProfile profile}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_box_autoadd_server_profile(profile, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 70, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSaveServerConstMeta,
      argValues: [dbPath, profile],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSaveServerConstMeta => const TaskConstMeta(
        debugName: "save_server",
        argNames: ["dbPath", "profile"],
      );

  @override
  Future<SeriesDetailRow?> crateFfiBridgeSeriesDetail(
      {required String dbPath,
      required String serverId,
      required String seriesId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 71, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_series_detail_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSeriesDetailConstMeta,
      argValues: [dbPath, serverId, seriesId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSeriesDetailConstMeta => const TaskConstMeta(
        debugName: "series_detail",
        argNames: ["dbPath", "serverId", "seriesId"],
      );

  @override
  Future<String?> crateFfiBridgeSeriesReadOverride(
      {required String dbPath,
      required String serverId,
      required String seriesId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 72, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSeriesReadOverrideConstMeta,
      argValues: [dbPath, serverId, seriesId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSeriesReadOverrideConstMeta =>
      const TaskConstMeta(
        debugName: "series_read_override",
        argNames: ["dbPath", "serverId", "seriesId"],
      );

  @override
  Future<ReadTargetRow> crateFfiBridgeSeriesReadTarget(
      {required String dbPath,
      required String serverId,
      required String seriesId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 73, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_read_target_row,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSeriesReadTargetConstMeta,
      argValues: [dbPath, serverId, seriesId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSeriesReadTargetConstMeta =>
      const TaskConstMeta(
        debugName: "series_read_target",
        argNames: ["dbPath", "serverId", "seriesId"],
      );

  @override
  Future<void> crateFfiBridgeSetActiveServer(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 74, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSetActiveServerConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSetActiveServerConstMeta =>
      const TaskConstMeta(
        debugName: "set_active_server",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<void> crateFfiBridgeSetReadProgress(
      {required String dbPath,
      required String serverId,
      required String bookId,
      required PlatformInt64 page,
      required bool completed}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(bookId, serializer);
        sse_encode_i_64(page, serializer);
        sse_encode_bool(completed, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 75, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSetReadProgressConstMeta,
      argValues: [dbPath, serverId, bookId, page, completed],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSetReadProgressConstMeta =>
      const TaskConstMeta(
        debugName: "set_read_progress",
        argNames: ["dbPath", "serverId", "bookId", "page", "completed"],
      );

  @override
  Future<String?> crateFfiBridgeSetSeriesReadOverride(
      {required String dbPath,
      required String serverId,
      required String seriesId,
      String? mode,
      String? direction}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(seriesId, serializer);
        sse_encode_opt_String(mode, serializer);
        sse_encode_opt_String(direction, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 76, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSetSeriesReadOverrideConstMeta,
      argValues: [dbPath, serverId, seriesId, mode, direction],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSetSeriesReadOverrideConstMeta =>
      const TaskConstMeta(
        debugName: "set_series_read_override",
        argNames: ["dbPath", "serverId", "seriesId", "mode", "direction"],
      );

  @override
  Future<bool> crateFfiBridgeShouldReconcile(
      {required String dbPath,
      required String serverId,
      required String trigger}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(trigger, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 77, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_bool,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeShouldReconcileConstMeta,
      argValues: [dbPath, serverId, trigger],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeShouldReconcileConstMeta =>
      const TaskConstMeta(
        debugName: "should_reconcile",
        argNames: ["dbPath", "serverId", "trigger"],
      );

  @override
  Future<SsePollResult?> crateFfiBridgeSsePoll(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey,
      required String stateJson}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        sse_encode_String(stateJson, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 78, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_opt_box_autoadd_sse_poll_result,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSsePollConstMeta,
      argValues: [dbPath, serverId, baseUrl, apiKey, stateJson],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSsePollConstMeta => const TaskConstMeta(
        debugName: "sse_poll",
        argNames: ["dbPath", "serverId", "baseUrl", "apiKey", "stateJson"],
      );

  @override
  Future<String> crateFfiBridgeSseReconciled(
      {required String dbPath, required String stateJson}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(stateJson, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 79, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSseReconciledConstMeta,
      argValues: [dbPath, stateJson],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSseReconciledConstMeta =>
      const TaskConstMeta(
        debugName: "sse_reconciled",
        argNames: ["dbPath", "stateJson"],
      );

  @override
  Future<String> crateFfiBridgeSseResume(
      {required String dbPath, required String stateJson}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(stateJson, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 80, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_String,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSseResumeConstMeta,
      argValues: [dbPath, stateJson],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSseResumeConstMeta => const TaskConstMeta(
        debugName: "sse_resume",
        argNames: ["dbPath", "stateJson"],
      );

  @override
  Future<void> crateFfiBridgeSseStop(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 81, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_unit,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSseStopConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSseStopConstMeta => const TaskConstMeta(
        debugName: "sse_stop",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<List<EntitySyncState>> crateFfiBridgeSyncStates(
      {required String dbPath, required String serverId}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 82, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_entity_sync_state,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeSyncStatesConstMeta,
      argValues: [dbPath, serverId],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeSyncStatesConstMeta => const TaskConstMeta(
        debugName: "sync_states",
        argNames: ["dbPath", "serverId"],
      );

  @override
  Future<ConnectionResult> crateFfiBridgeTestConnection(
      {required String baseUrl, required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 83, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_connection_result,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeTestConnectionConstMeta,
      argValues: [baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeTestConnectionConstMeta =>
      const TaskConstMeta(
        debugName: "test_connection",
        argNames: ["baseUrl", "apiKey"],
      );

  @override
  Future<List<Tombstone>> crateFfiBridgeTombstones(
      {required String dbPath,
      required String serverId,
      required String entityType}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(entityType, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 84, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_list_tombstone,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeTombstonesConstMeta,
      argValues: [dbPath, serverId, entityType],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeTombstonesConstMeta => const TaskConstMeta(
        debugName: "tombstones",
        argNames: ["dbPath", "serverId", "entityType"],
      );

  @override
  Future<UploadOutcomeDto> crateFfiBridgeUploadOutbox(
      {required String dbPath,
      required String serverId,
      required String baseUrl,
      required String apiKey}) {
    return handler.executeNormal(NormalTask(
      callFfi: (port_) {
        final serializer = SseSerializer(generalizedFrbRustBinding);
        sse_encode_String(dbPath, serializer);
        sse_encode_String(serverId, serializer);
        sse_encode_String(baseUrl, serializer);
        sse_encode_String(apiKey, serializer);
        pdeCallFfi(generalizedFrbRustBinding, serializer,
            funcId: 85, port: port_);
      },
      codec: SseCodec(
        decodeSuccessData: sse_decode_upload_outcome_dto,
        decodeErrorData: sse_decode_core_error,
      ),
      constMeta: kCrateFfiBridgeUploadOutboxConstMeta,
      argValues: [dbPath, serverId, baseUrl, apiKey],
      apiImpl: this,
    ));
  }

  TaskConstMeta get kCrateFfiBridgeUploadOutboxConstMeta => const TaskConstMeta(
        debugName: "upload_outbox",
        argNames: ["dbPath", "serverId", "baseUrl", "apiKey"],
      );

  @protected
  Map<String, String> dco_decode_Map_String_String_None(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return Map.fromEntries(dco_decode_list_record_string_string(raw)
        .map((e) => MapEntry(e.$1, e.$2)));
  }

  @protected
  String dco_decode_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as String;
  }

  @protected
  ApiPolicyDto dco_decode_api_policy_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return ApiPolicyDto(
      contractVersion: dco_decode_String(arr[0]),
      snapshotVersion: dco_decode_String(arr[1]),
      minServerVersion: dco_decode_String(arr[2]),
    );
  }

  @protected
  AuthStateDto dco_decode_auth_state_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return AuthStateDto(
      serverId: dco_decode_String(arr[0]),
      state: dco_decode_String(arr[1]),
      at: dco_decode_String(arr[2]),
    );
  }

  @protected
  AuthType dco_decode_auth_type(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return AuthType.values[raw as int];
  }

  @protected
  AuthorRow dco_decode_author_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return AuthorRow(
      name: dco_decode_String(arr[0]),
      role: dco_decode_String(arr[1]),
    );
  }

  @protected
  BookDetailRow dco_decode_book_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 19)
      throw Exception('unexpected arr length: expect 19 but see ${arr.length}');
    return BookDetailRow(
      serverId: dco_decode_String(arr[0]),
      remoteId: dco_decode_String(arr[1]),
      seriesId: dco_decode_String(arr[2]),
      seriesTitle: dco_decode_opt_String(arr[3]),
      title: dco_decode_String(arr[4]),
      number: dco_decode_opt_String(arr[5]),
      numberSort: dco_decode_opt_box_autoadd_f_64(arr[6]),
      summary: dco_decode_opt_String(arr[7]),
      isbn: dco_decode_opt_String(arr[8]),
      releaseDate: dco_decode_opt_String(arr[9]),
      mediaType: dco_decode_opt_String(arr[10]),
      pagesCount: dco_decode_opt_box_autoadd_i_64(arr[11]),
      fileSize: dco_decode_opt_box_autoadd_i_64(arr[12]),
      createdAt: dco_decode_opt_String(arr[13]),
      lastModified: dco_decode_opt_String(arr[14]),
      tags: dco_decode_list_String(arr[15]),
      authors: dco_decode_list_author_row(arr[16]),
      progressPage: dco_decode_opt_box_autoadd_i_64(arr[17]),
      progressCompleted: dco_decode_bool(arr[18]),
    );
  }

  @protected
  BookPageResult dco_decode_book_page_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return BookPageResult(
      items: dco_decode_list_book_row(arr[0]),
      total: dco_decode_i_64(arr[1]),
    );
  }

  @protected
  BookRow dco_decode_book_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 15)
      throw Exception('unexpected arr length: expect 15 but see ${arr.length}');
    return BookRow(
      serverId: dco_decode_String(arr[0]),
      remoteId: dco_decode_String(arr[1]),
      seriesId: dco_decode_String(arr[2]),
      seriesTitle: dco_decode_opt_String(arr[3]),
      title: dco_decode_String(arr[4]),
      number: dco_decode_opt_String(arr[5]),
      numberSort: dco_decode_opt_box_autoadd_f_64(arr[6]),
      fileSize: dco_decode_opt_box_autoadd_i_64(arr[7]),
      mediaType: dco_decode_opt_String(arr[8]),
      pagesCount: dco_decode_opt_box_autoadd_i_64(arr[9]),
      createdAt: dco_decode_opt_String(arr[10]),
      lastModified: dco_decode_opt_String(arr[11]),
      progressPage: dco_decode_opt_box_autoadd_i_64(arr[12]),
      progressCompleted: dco_decode_bool(arr[13]),
      ftsRowid: dco_decode_opt_box_autoadd_i_64(arr[14]),
    );
  }

  @protected
  bool dco_decode_bool(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as bool;
  }

  @protected
  BootstrapSummary dco_decode_bootstrap_summary(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return BootstrapSummary(
      serverId: dco_decode_String(arr[0]),
      syncedSeries: dco_decode_usize(arr[1]),
      totalElements: dco_decode_i_64(arr[2]),
      hasMorePages: dco_decode_bool(arr[3]),
    );
  }

  @protected
  BookDetailRow dco_decode_box_autoadd_book_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_book_detail_row(raw);
  }

  @protected
  bool dco_decode_box_autoadd_bool(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as bool;
  }

  @protected
  BuildInfo dco_decode_box_autoadd_build_info(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_build_info(raw);
  }

  @protected
  CollectionDetailRow dco_decode_box_autoadd_collection_detail_row(
      dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_collection_detail_row(raw);
  }

  @protected
  DeviceProfileDto dco_decode_box_autoadd_device_profile_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_device_profile_dto(raw);
  }

  @protected
  DownloadPumpDto dco_decode_box_autoadd_download_pump_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_download_pump_dto(raw);
  }

  @protected
  double dco_decode_box_autoadd_f_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as double;
  }

  @protected
  GitCommit dco_decode_box_autoadd_git_commit(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_git_commit(raw);
  }

  @protected
  GitInfo dco_decode_box_autoadd_git_info(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_git_info(raw);
  }

  @protected
  PlatformInt64 dco_decode_box_autoadd_i_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_i_64(raw);
  }

  @protected
  LibraryCountRow dco_decode_box_autoadd_library_count_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_library_count_row(raw);
  }

  @protected
  ReaderSettingsDto dco_decode_box_autoadd_reader_settings_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_reader_settings_dto(raw);
  }

  @protected
  ReadlistDetailRow dco_decode_box_autoadd_readlist_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_readlist_detail_row(raw);
  }

  @protected
  SeriesDetailRow dco_decode_box_autoadd_series_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_series_detail_row(raw);
  }

  @protected
  ServerProfile dco_decode_box_autoadd_server_profile(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_server_profile(raw);
  }

  @protected
  SsePollResult dco_decode_box_autoadd_sse_poll_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_sse_poll_result(raw);
  }

  @protected
  BuildInfo dco_decode_build_info(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return BuildInfo(
      artifact: dco_decode_opt_String(arr[0]),
      name: dco_decode_opt_String(arr[1]),
      time: dco_decode_opt_String(arr[2]),
      version: dco_decode_opt_String(arr[3]),
      group: dco_decode_opt_String(arr[4]),
    );
  }

  @protected
  CacheCleanupDto dco_decode_cache_cleanup_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return CacheCleanupDto(
      ghostRows: dco_decode_i_64(arr[0]),
      orphanFiles: dco_decode_i_64(arr[1]),
      staleParts: dco_decode_i_64(arr[2]),
      corrupt: dco_decode_i_64(arr[3]),
      kindRepaired: dco_decode_i_64(arr[4]),
      evicted: dco_decode_i_64(arr[5]),
      freedBytes: dco_decode_i_64(arr[6]),
      bytesAfter: dco_decode_i_64(arr[7]),
    );
  }

  @protected
  CacheStatsDto dco_decode_cache_stats_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 14)
      throw Exception('unexpected arr length: expect 14 but see ${arr.length}');
    return CacheStatsDto(
      pageBytes: dco_decode_i_64(arr[0]),
      prefetchBytes: dco_decode_i_64(arr[1]),
      downloadBytes: dco_decode_i_64(arr[2]),
      poolBudgetBytes: dco_decode_i_64(arr[3]),
      memoryBytes: dco_decode_i_64(arr[4]),
      memoryPeakBytes: dco_decode_i_64(arr[5]),
      memoryEntries: dco_decode_i_64(arr[6]),
      memoryHits: dco_decode_i_64(arr[7]),
      memoryMisses: dco_decode_i_64(arr[8]),
      memoryEvictions: dco_decode_i_64(arr[9]),
      memoryRefused: dco_decode_i_64(arr[10]),
      diskBytes: dco_decode_i_64(arr[11]),
      ledgerBytes: dco_decode_i_64(arr[12]),
      openReaders: dco_decode_i_64(arr[13]),
    );
  }

  @protected
  CollectionDetailRow dco_decode_collection_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return CollectionDetailRow(
      remoteId: dco_decode_String(arr[0]),
      name: dco_decode_String(arr[1]),
      ordered: dco_decode_bool(arr[2]),
      filtered: dco_decode_bool(arr[3]),
      createdDate: dco_decode_opt_String(arr[4]),
      lastModifiedDate: dco_decode_opt_String(arr[5]),
      members: dco_decode_series_page_result(arr[6]),
    );
  }

  @protected
  CollectionPageResult dco_decode_collection_page_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return CollectionPageResult(
      items: dco_decode_list_collection_row(arr[0]),
      total: dco_decode_i_64(arr[1]),
    );
  }

  @protected
  CollectionRef dco_decode_collection_ref(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return CollectionRef(
      remoteId: dco_decode_String(arr[0]),
      name: dco_decode_String(arr[1]),
    );
  }

  @protected
  CollectionRow dco_decode_collection_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return CollectionRow(
      serverId: dco_decode_String(arr[0]),
      remoteId: dco_decode_String(arr[1]),
      name: dco_decode_String(arr[2]),
      ordered: dco_decode_bool(arr[3]),
      filtered: dco_decode_bool(arr[4]),
      createdDate: dco_decode_opt_String(arr[5]),
      lastModifiedDate: dco_decode_opt_String(arr[6]),
    );
  }

  @protected
  ConnectionResult dco_decode_connection_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return ConnectionResult(
      serverInfo: dco_decode_server_info(arr[0]),
      serverVersion: dco_decode_opt_String(arr[1]),
      libraries: dco_decode_list_library(arr[2]),
      capabilities: dco_decode_list_String(arr[3]),
    );
  }

  @protected
  ContinueReadingRow dco_decode_continue_reading_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 9)
      throw Exception('unexpected arr length: expect 9 but see ${arr.length}');
    return ContinueReadingRow(
      bookId: dco_decode_String(arr[0]),
      bookTitle: dco_decode_String(arr[1]),
      number: dco_decode_opt_String(arr[2]),
      seriesId: dco_decode_String(arr[3]),
      seriesName: dco_decode_String(arr[4]),
      page: dco_decode_opt_box_autoadd_i_64(arr[5]),
      totalPages: dco_decode_opt_box_autoadd_i_64(arr[6]),
      progressPct: dco_decode_opt_box_autoadd_i_64(arr[7]),
      localUpdatedAt: dco_decode_opt_String(arr[8]),
    );
  }

  @protected
  CoreError dco_decode_core_error(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return CoreError(
      code: dco_decode_error_code(arr[0]),
      message: dco_decode_String(arr[1]),
      retryable: dco_decode_bool(arr[2]),
      needsUser: dco_decode_bool(arr[3]),
    );
  }

  @protected
  DbHealth dco_decode_db_health(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 10)
      throw Exception('unexpected arr length: expect 10 but see ${arr.length}');
    return DbHealth(
      schemaVersion: dco_decode_i_64(arr[0]),
      integrity: dco_decode_String(arr[1]),
      journalMode: dco_decode_String(arr[2]),
      pageSize: dco_decode_i_64(arr[3]),
      pageCount: dco_decode_i_64(arr[4]),
      freelistCount: dco_decode_i_64(arr[5]),
      busyTimeoutMs: dco_decode_i_64(arr[6]),
      foreignKeysOn: dco_decode_bool(arr[7]),
      fileBytes: dco_decode_i_64(arr[8]),
      tables: dco_decode_list_table_rows(arr[9]),
    );
  }

  @protected
  DeviceProfileDto dco_decode_device_profile_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 6)
      throw Exception('unexpected arr length: expect 6 but see ${arr.length}');
    return DeviceProfileDto(
      deviceMemoryBytes: dco_decode_i_64(arr[0]),
      cacheBudgetBytes: dco_decode_i_64(arr[1]),
      avgPageBytesHint: dco_decode_i_64(arr[2]),
      decodedPageBytes: dco_decode_i_64(arr[3]),
      network: dco_decode_String(arr[4]),
      stable: dco_decode_bool(arr[5]),
    );
  }

  @protected
  DiagnosticsDto dco_decode_diagnostics_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 11)
      throw Exception('unexpected arr length: expect 11 but see ${arr.length}');
    return DiagnosticsDto(
      serverId: dco_decode_String(arr[0]),
      db: dco_decode_db_health(arr[1]),
      auth: dco_decode_auth_state_dto(arr[2]),
      outboxQueuedRows: dco_decode_i_64(arr[3]),
      sync_: dco_decode_list_entity_sync_state(arr[4]),
      outbox: dco_decode_outbox_status_dto(arr[5]),
      cache: dco_decode_cache_stats_dto(arr[6]),
      storage: dco_decode_storage_dto(arr[7]),
      queue: dco_decode_list_queue_state_count_dto(arr[8]),
      policy: dco_decode_api_policy_dto(arr[9]),
      log: dco_decode_log_stats(arr[10]),
    );
  }

  @protected
  DownloadBookDto dco_decode_download_book_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 15)
      throw Exception('unexpected arr length: expect 15 but see ${arr.length}');
    return DownloadBookDto(
      serverId: dco_decode_String(arr[0]),
      bookId: dco_decode_String(arr[1]),
      title: dco_decode_String(arr[2]),
      seriesTitle: dco_decode_String(arr[3]),
      state: dco_decode_String(arr[4]),
      pagesTotal: dco_decode_i_64(arr[5]),
      pagesDone: dco_decode_i_64(arr[6]),
      bytesTotal: dco_decode_i_64(arr[7]),
      bytesDone: dco_decode_i_64(arr[8]),
      position: dco_decode_i_64(arr[9]),
      lastError: dco_decode_String(arr[10]),
      nextRetryAt: dco_decode_String(arr[11]),
      remoteLastModified: dco_decode_String(arr[12]),
      allowCellular: dco_decode_bool(arr[13]),
      stale: dco_decode_bool(arr[14]),
    );
  }

  @protected
  DownloadDeleteDto dco_decode_download_delete_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return DownloadDeleteDto(
      books: dco_decode_i_64(arr[0]),
      files: dco_decode_i_64(arr[1]),
      freedBytes: dco_decode_i_64(arr[2]),
    );
  }

  @protected
  DownloadPumpDto dco_decode_download_pump_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 16)
      throw Exception('unexpected arr length: expect 16 but see ${arr.length}');
    return DownloadPumpDto(
      book: dco_decode_String(arr[0]),
      state: dco_decode_String(arr[1]),
      served: dco_decode_i_64(arr[2]),
      failedPages: dco_decode_i_64(arr[3]),
      bytesWritten: dco_decode_i_64(arr[4]),
      pagesDone: dco_decode_i_64(arr[5]),
      pagesTotal: dco_decode_i_64(arr[6]),
      stopReason: dco_decode_String(arr[7]),
      nextInMs: dco_decode_i_64(arr[8]),
      pumpMs: dco_decode_i_64(arr[9]),
      lastError: dco_decode_String(arr[10]),
      repairs: dco_decode_i_64(arr[11]),
      partsSwept: dco_decode_i_64(arr[12]),
      adopted: dco_decode_i_64(arr[13]),
      ghostRows: dco_decode_i_64(arr[14]),
      queueActive: dco_decode_bool(arr[15]),
    );
  }

  @protected
  DownloadSweepDto dco_decode_download_sweep_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 12)
      throw Exception('unexpected arr length: expect 12 but see ${arr.length}');
    return DownloadSweepDto(
      books: dco_decode_i_64(arr[0]),
      staleParts: dco_decode_i_64(arr[1]),
      ghostRows: dco_decode_i_64(arr[2]),
      corrupt: dco_decode_i_64(arr[3]),
      sizeMismatch: dco_decode_i_64(arr[4]),
      adoptedFiles: dco_decode_i_64(arr[5]),
      countersRepaired: dco_decode_i_64(arr[6]),
      manifestsRewritten: dco_decode_i_64(arr[7]),
      pagesRemoved: dco_decode_i_64(arr[8]),
      unownedBooks: dco_decode_i_64(arr[9]),
      unownedBytes: dco_decode_i_64(arr[10]),
      freedBytes: dco_decode_i_64(arr[11]),
    );
  }

  @protected
  EntitySyncState dco_decode_entity_sync_state(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return EntitySyncState(
      serverId: dco_decode_String(arr[0]),
      entityType: dco_decode_String(arr[1]),
      lastSyncAt: dco_decode_opt_String(arr[2]),
      syncCursor: dco_decode_opt_String(arr[3]),
      syncStatus: dco_decode_String(arr[4]),
      lastError: dco_decode_opt_String(arr[5]),
      lastFullSync: dco_decode_opt_String(arr[6]),
      lastSuccessfulSync: dco_decode_opt_String(arr[7]),
    );
  }

  @protected
  ErrorCode dco_decode_error_code(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return ErrorCode.values[raw as int];
  }

  @protected
  double dco_decode_f_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as double;
  }

  @protected
  FilterOptions dco_decode_filter_options(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return FilterOptions(
      tags: dco_decode_list_String(arr[0]),
      genres: dco_decode_list_String(arr[1]),
      statuses: dco_decode_list_String(arr[2]),
    );
  }

  @protected
  FullSyncSummary dco_decode_full_sync_summary(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 11)
      throw Exception('unexpected arr length: expect 11 but see ${arr.length}');
    return FullSyncSummary(
      serverId: dco_decode_String(arr[0]),
      libraries: dco_decode_usize(arr[1]),
      series: dco_decode_usize(arr[2]),
      books: dco_decode_usize(arr[3]),
      collections: dco_decode_usize(arr[4]),
      readlists: dco_decode_usize(arr[5]),
      readProgress: dco_decode_usize(arr[6]),
      seriesPages: dco_decode_u_32(arr[7]),
      bookPages: dco_decode_u_32(arr[8]),
      skippedSteps: dco_decode_list_String(arr[9]),
      resumedSteps: dco_decode_list_String(arr[10]),
    );
  }

  @protected
  GitCommit dco_decode_git_commit(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return GitCommit(
      id: dco_decode_opt_String(arr[0]),
      time: dco_decode_opt_String(arr[1]),
    );
  }

  @protected
  GitInfo dco_decode_git_info(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return GitInfo(
      branch: dco_decode_opt_String(arr[0]),
      commit: dco_decode_opt_box_autoadd_git_commit(arr[1]),
    );
  }

  @protected
  int dco_decode_i_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  PlatformInt64 dco_decode_i_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dcoDecodeI64(raw);
  }

  @protected
  Library dco_decode_library(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return Library(
      id: dco_decode_String(arr[0]),
      name: dco_decode_String(arr[1]),
      root: dco_decode_String(arr[2]),
      unavailable: dco_decode_opt_box_autoadd_bool(arr[3]),
    );
  }

  @protected
  LibraryCountRow dco_decode_library_count_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return LibraryCountRow(
      remoteId: dco_decode_String(arr[0]),
      name: dco_decode_String(arr[1]),
      root: dco_decode_opt_String(arr[2]),
      unavailable: dco_decode_bool(arr[3]),
      seriesCount: dco_decode_i_64(arr[4]),
      bookCount: dco_decode_i_64(arr[5]),
      readCount: dco_decode_i_64(arr[6]),
    );
  }

  @protected
  List<String> dco_decode_list_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_String).toList();
  }

  @protected
  List<AuthorRow> dco_decode_list_author_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_author_row).toList();
  }

  @protected
  List<BookRow> dco_decode_list_book_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_book_row).toList();
  }

  @protected
  List<CollectionRef> dco_decode_list_collection_ref(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_collection_ref).toList();
  }

  @protected
  List<CollectionRow> dco_decode_list_collection_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_collection_row).toList();
  }

  @protected
  List<ContinueReadingRow> dco_decode_list_continue_reading_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_continue_reading_row).toList();
  }

  @protected
  List<DownloadBookDto> dco_decode_list_download_book_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_download_book_dto).toList();
  }

  @protected
  List<EntitySyncState> dco_decode_list_entity_sync_state(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_entity_sync_state).toList();
  }

  @protected
  List<Library> dco_decode_list_library(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_library).toList();
  }

  @protected
  List<LibraryCountRow> dco_decode_list_library_count_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_library_count_row).toList();
  }

  @protected
  List<Uint32List> dco_decode_list_list_prim_u_32_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_list_prim_u_32_strict)
        .toList();
  }

  @protected
  List<LogRecord> dco_decode_list_log_record(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_log_record).toList();
  }

  @protected
  List<OutboxEntryDto> dco_decode_list_outbox_entry_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_outbox_entry_dto).toList();
  }

  @protected
  Uint32List dco_decode_list_prim_u_32_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as Uint32List;
  }

  @protected
  Uint8List dco_decode_list_prim_u_8_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as Uint8List;
  }

  @protected
  List<QueueStateCountDto> dco_decode_list_queue_state_count_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_queue_state_count_dto)
        .toList();
  }

  @protected
  List<ReadlistRow> dco_decode_list_readlist_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_readlist_row).toList();
  }

  @protected
  List<(String, String)> dco_decode_list_record_string_string(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_record_string_string).toList();
  }

  @protected
  List<SeriesRow> dco_decode_list_series_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_series_row).toList();
  }

  @protected
  List<ServerProfile> dco_decode_list_server_profile(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_server_profile).toList();
  }

  @protected
  List<StorageBookDto> dco_decode_list_storage_book_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_storage_book_dto).toList();
  }

  @protected
  List<TableRows> dco_decode_list_table_rows(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_table_rows).toList();
  }

  @protected
  List<ThumbnailRow> dco_decode_list_thumbnail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_thumbnail_row).toList();
  }

  @protected
  List<Tombstone> dco_decode_list_tombstone(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_tombstone).toList();
  }

  @protected
  LogRecord dco_decode_log_record(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return LogRecord(
      level: dco_decode_String(arr[0]),
      target: dco_decode_String(arr[1]),
      message: dco_decode_String(arr[2]),
      at: dco_decode_String(arr[3]),
    );
  }

  @protected
  LogStats dco_decode_log_stats(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 11)
      throw Exception('unexpected arr length: expect 11 but see ${arr.length}');
    return LogStats(
      installed: dco_decode_bool(arr[0]),
      capacity: dco_decode_i_64(arr[1]),
      retained: dco_decode_i_64(arr[2]),
      dropped: dco_decode_i_64(arr[3]),
      errors: dco_decode_i_64(arr[4]),
      warnings: dco_decode_i_64(arr[5]),
      info: dco_decode_i_64(arr[6]),
      debug: dco_decode_i_64(arr[7]),
      trace: dco_decode_i_64(arr[8]),
      lastError: dco_decode_opt_String(arr[9]),
      maxLevel: dco_decode_String(arr[10]),
    );
  }

  @protected
  String? dco_decode_opt_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_String(raw);
  }

  @protected
  BookDetailRow? dco_decode_opt_box_autoadd_book_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_book_detail_row(raw);
  }

  @protected
  bool? dco_decode_opt_box_autoadd_bool(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_bool(raw);
  }

  @protected
  BuildInfo? dco_decode_opt_box_autoadd_build_info(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_build_info(raw);
  }

  @protected
  CollectionDetailRow? dco_decode_opt_box_autoadd_collection_detail_row(
      dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_collection_detail_row(raw);
  }

  @protected
  DownloadPumpDto? dco_decode_opt_box_autoadd_download_pump_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_download_pump_dto(raw);
  }

  @protected
  double? dco_decode_opt_box_autoadd_f_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_f_64(raw);
  }

  @protected
  GitCommit? dco_decode_opt_box_autoadd_git_commit(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_git_commit(raw);
  }

  @protected
  GitInfo? dco_decode_opt_box_autoadd_git_info(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_git_info(raw);
  }

  @protected
  PlatformInt64? dco_decode_opt_box_autoadd_i_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_i_64(raw);
  }

  @protected
  LibraryCountRow? dco_decode_opt_box_autoadd_library_count_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_library_count_row(raw);
  }

  @protected
  ReadlistDetailRow? dco_decode_opt_box_autoadd_readlist_detail_row(
      dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_readlist_detail_row(raw);
  }

  @protected
  SeriesDetailRow? dco_decode_opt_box_autoadd_series_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_series_detail_row(raw);
  }

  @protected
  ServerProfile? dco_decode_opt_box_autoadd_server_profile(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_server_profile(raw);
  }

  @protected
  SsePollResult? dco_decode_opt_box_autoadd_sse_poll_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_sse_poll_result(raw);
  }

  @protected
  OutboxEntryDto dco_decode_outbox_entry_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return OutboxEntryDto(
      id: dco_decode_String(arr[0]),
      entityId: dco_decode_String(arr[1]),
      mutationType: dco_decode_String(arr[2]),
      retryCount: dco_decode_i_64(arr[3]),
      lastError: dco_decode_opt_String(arr[4]),
      state: dco_decode_String(arr[5]),
      nextRetryAt: dco_decode_opt_String(arr[6]),
      createdAt: dco_decode_String(arr[7]),
    );
  }

  @protected
  OutboxStatusDto dco_decode_outbox_status_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 6)
      throw Exception('unexpected arr length: expect 6 but see ${arr.length}');
    return OutboxStatusDto(
      serverId: dco_decode_String(arr[0]),
      pending: dco_decode_i_64(arr[1]),
      waiting: dco_decode_i_64(arr[2]),
      failed: dco_decode_i_64(arr[3]),
      total: dco_decode_i_64(arr[4]),
      failedEntries: dco_decode_list_outbox_entry_dto(arr[5]),
    );
  }

  @protected
  QueueStateCountDto dco_decode_queue_state_count_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 6)
      throw Exception('unexpected arr length: expect 6 but see ${arr.length}');
    return QueueStateCountDto(
      state: dco_decode_String(arr[0]),
      books: dco_decode_i_64(arr[1]),
      pagesDone: dco_decode_i_64(arr[2]),
      pagesTotal: dco_decode_i_64(arr[3]),
      bytesDone: dco_decode_i_64(arr[4]),
      bytesTotal: dco_decode_i_64(arr[5]),
    );
  }

  @protected
  ReadTargetRow dco_decode_read_target_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return ReadTargetRow(
      book: dco_decode_book_row(arr[0]),
      intent: dco_decode_String(arr[1]),
      position: dco_decode_i_64(arr[2]),
      bookCount: dco_decode_opt_box_autoadd_i_64(arr[3]),
      complete: dco_decode_bool(arr[4]),
    );
  }

  @protected
  ReaderBookDto dco_decode_reader_book_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 10)
      throw Exception('unexpected arr length: expect 10 but see ${arr.length}');
    return ReaderBookDto(
      serverId: dco_decode_String(arr[0]),
      bookId: dco_decode_String(arr[1]),
      pageCount: dco_decode_i_64(arr[2]),
      paged: dco_decode_bool(arr[3]),
      reflowable: dco_decode_bool(arr[4]),
      fallback: dco_decode_opt_String(arr[5]),
      fromMirror: dco_decode_bool(arr[6]),
      startPage: dco_decode_i_64(arr[7]),
      startPageOffsetRatio: dco_decode_opt_box_autoadd_f_64(arr[8]),
      layout: dco_decode_reader_layout_dto(arr[9]),
    );
  }

  @protected
  ReaderLayoutDto dco_decode_reader_layout_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 13)
      throw Exception('unexpected arr length: expect 13 but see ${arr.length}');
    return ReaderLayoutDto(
      spreads: dco_decode_list_list_prim_u_32_strict(arr[0]),
      spread: dco_decode_i_64(arr[1]),
      page: dco_decode_i_64(arr[2]),
      axis: dco_decode_String(arr[3]),
      reversed: dco_decode_bool(arr[4]),
      advanceSwipe: dco_decode_String(arr[5]),
      retreatSwipe: dco_decode_String(arr[6]),
      tapNext: dco_decode_String(arr[7]),
      tapPrev: dco_decode_String(arr[8]),
      mode: dco_decode_String(arr[9]),
      direction: dco_decode_String(arr[10]),
      pageGap: dco_decode_i_64(arr[11]),
      background: dco_decode_String(arr[12]),
    );
  }

  @protected
  ReaderSettingsDto dco_decode_reader_settings_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 12)
      throw Exception('unexpected arr length: expect 12 but see ${arr.length}');
    return ReaderSettingsDto(
      mode: dco_decode_String(arr[0]),
      direction: dco_decode_String(arr[1]),
      firstPageSingle: dco_decode_bool(arr[2]),
      pageGap: dco_decode_i_64(arr[3]),
      background: dco_decode_String(arr[4]),
      keepScreenAwake: dco_decode_bool(arr[5]),
      brightness: dco_decode_opt_box_autoadd_f_64(arr[6]),
      restorePosition: dco_decode_bool(arr[7]),
      prefetchForward: dco_decode_i_64(arr[8]),
      prefetchBack: dco_decode_i_64(arr[9]),
      prefetchCap: dco_decode_i_64(arr[10]),
      volumeKeysEnabled: dco_decode_bool(arr[11]),
    );
  }

  @protected
  ReaderTurnDto dco_decode_reader_turn_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return ReaderTurnDto(
      page: dco_decode_i_64(arr[0]),
      spread: dco_decode_i_64(arr[1]),
      uploadNow: dco_decode_bool(arr[2]),
    );
  }

  @protected
  ReaderWindowDto dco_decode_reader_window_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 11)
      throw Exception('unexpected arr length: expect 11 but see ${arr.length}');
    return ReaderWindowDto(
      forward: dco_decode_i_64(arr[0]),
      back: dco_decode_i_64(arr[1]),
      cap: dco_decode_i_64(arr[2]),
      memoryBudgetBytes: dco_decode_i_64(arr[3]),
      inFlight: dco_decode_i_64(arr[4]),
      decodeSlots: dco_decode_i_64(arr[5]),
      avgPageBytes: dco_decode_i_64(arr[6]),
      pagesPerSpread: dco_decode_i_64(arr[7]),
      poolBudgetBytes: dco_decode_i_64(arr[8]),
      sweptFreedBytes: dco_decode_i_64(arr[9]),
      sweptCorrupt: dco_decode_i_64(arr[10]),
    );
  }

  @protected
  ReadlistDetailRow dco_decode_readlist_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return ReadlistDetailRow(
      remoteId: dco_decode_String(arr[0]),
      name: dco_decode_String(arr[1]),
      summary: dco_decode_opt_String(arr[2]),
      ordered: dco_decode_bool(arr[3]),
      filtered: dco_decode_bool(arr[4]),
      createdDate: dco_decode_opt_String(arr[5]),
      lastModifiedDate: dco_decode_opt_String(arr[6]),
      books: dco_decode_book_page_result(arr[7]),
    );
  }

  @protected
  ReadlistPageResult dco_decode_readlist_page_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return ReadlistPageResult(
      items: dco_decode_list_readlist_row(arr[0]),
      total: dco_decode_i_64(arr[1]),
    );
  }

  @protected
  ReadlistRow dco_decode_readlist_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return ReadlistRow(
      serverId: dco_decode_String(arr[0]),
      remoteId: dco_decode_String(arr[1]),
      name: dco_decode_String(arr[2]),
      summary: dco_decode_opt_String(arr[3]),
      ordered: dco_decode_bool(arr[4]),
      filtered: dco_decode_bool(arr[5]),
      createdDate: dco_decode_opt_String(arr[6]),
      lastModifiedDate: dco_decode_opt_String(arr[7]),
    );
  }

  @protected
  ReconcileSummary dco_decode_reconcile_summary(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 24)
      throw Exception('unexpected arr length: expect 24 but see ${arr.length}');
    return ReconcileSummary(
      serverId: dco_decode_String(arr[0]),
      trigger: dco_decode_String(arr[1]),
      seriesUpserted: dco_decode_usize(arr[2]),
      seriesAdded: dco_decode_usize(arr[3]),
      seriesChanged: dco_decode_usize(arr[4]),
      seriesRemoved: dco_decode_usize(arr[5]),
      booksUpserted: dco_decode_usize(arr[6]),
      booksAdded: dco_decode_usize(arr[7]),
      booksChanged: dco_decode_usize(arr[8]),
      booksRemoved: dco_decode_usize(arr[9]),
      collectionsUpserted: dco_decode_usize(arr[10]),
      collectionsAdded: dco_decode_usize(arr[11]),
      collectionsChanged: dco_decode_usize(arr[12]),
      collectionsRemoved: dco_decode_usize(arr[13]),
      readlistsUpserted: dco_decode_usize(arr[14]),
      readlistsAdded: dco_decode_usize(arr[15]),
      readlistsChanged: dco_decode_usize(arr[16]),
      readlistsRemoved: dco_decode_usize(arr[17]),
      librariesUpserted: dco_decode_usize(arr[18]),
      librariesRemoved: dco_decode_usize(arr[19]),
      readProgress: dco_decode_usize(arr[20]),
      pagesSwept: dco_decode_u_32(arr[21]),
      orphanedCovers: dco_decode_list_String(arr[22]),
      clean: dco_decode_bool(arr[23]),
    );
  }

  @protected
  (String, String) dco_decode_record_string_string(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2) {
      throw Exception('Expected 2 elements, got ${arr.length}');
    }
    return (
      dco_decode_String(arr[0]),
      dco_decode_String(arr[1]),
    );
  }

  @protected
  SeriesDetailRow dco_decode_series_detail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 22)
      throw Exception('unexpected arr length: expect 22 but see ${arr.length}');
    return SeriesDetailRow(
      serverId: dco_decode_String(arr[0]),
      remoteId: dco_decode_String(arr[1]),
      libraryId: dco_decode_String(arr[2]),
      name: dco_decode_String(arr[3]),
      sortName: dco_decode_opt_String(arr[4]),
      status: dco_decode_opt_String(arr[5]),
      createdAt: dco_decode_opt_String(arr[6]),
      lastModified: dco_decode_opt_String(arr[7]),
      booksCount: dco_decode_opt_box_autoadd_i_64(arr[8]),
      booksReadCount: dco_decode_opt_box_autoadd_i_64(arr[9]),
      booksUnreadCount: dco_decode_opt_box_autoadd_i_64(arr[10]),
      booksInProgressCount: dco_decode_opt_box_autoadd_i_64(arr[11]),
      summary: dco_decode_opt_String(arr[12]),
      publisher: dco_decode_opt_String(arr[13]),
      readingDirection: dco_decode_opt_String(arr[14]),
      language: dco_decode_opt_String(arr[15]),
      ageRating: dco_decode_opt_String(arr[16]),
      totalBookCount: dco_decode_opt_box_autoadd_i_64(arr[17]),
      genres: dco_decode_list_String(arr[18]),
      tags: dco_decode_list_String(arr[19]),
      authors: dco_decode_list_author_row(arr[20]),
      collections: dco_decode_list_collection_ref(arr[21]),
    );
  }

  @protected
  SeriesPageResult dco_decode_series_page_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return SeriesPageResult(
      items: dco_decode_list_series_row(arr[0]),
      total: dco_decode_i_64(arr[1]),
    );
  }

  @protected
  SeriesRow dco_decode_series_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 13)
      throw Exception('unexpected arr length: expect 13 but see ${arr.length}');
    return SeriesRow(
      serverId: dco_decode_String(arr[0]),
      remoteId: dco_decode_String(arr[1]),
      libraryId: dco_decode_String(arr[2]),
      name: dco_decode_String(arr[3]),
      sortName: dco_decode_opt_String(arr[4]),
      status: dco_decode_opt_String(arr[5]),
      createdAt: dco_decode_opt_String(arr[6]),
      lastModified: dco_decode_opt_String(arr[7]),
      booksCount: dco_decode_opt_box_autoadd_i_64(arr[8]),
      booksReadCount: dco_decode_opt_box_autoadd_i_64(arr[9]),
      booksUnreadCount: dco_decode_opt_box_autoadd_i_64(arr[10]),
      booksInProgressCount: dco_decode_opt_box_autoadd_i_64(arr[11]),
      ftsRowid: dco_decode_opt_box_autoadd_i_64(arr[12]),
    );
  }

  @protected
  ServerInfo dco_decode_server_info(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return ServerInfo(
      build: dco_decode_opt_box_autoadd_build_info(arr[0]),
      git: dco_decode_opt_box_autoadd_git_info(arr[1]),
    );
  }

  @protected
  ServerProfile dco_decode_server_profile(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return ServerProfile(
      id: dco_decode_String(arr[0]),
      displayName: dco_decode_String(arr[1]),
      baseUrl: dco_decode_String(arr[2]),
      authType: dco_decode_auth_type(arr[3]),
      credentialRef: dco_decode_opt_String(arr[4]),
      capabilities: dco_decode_list_String(arr[5]),
      lastSuccessfulConnection: dco_decode_opt_String(arr[6]),
    );
  }

  @protected
  SsePollResult dco_decode_sse_poll_result(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 9)
      throw Exception('unexpected arr length: expect 9 but see ${arr.length}');
    return SsePollResult(
      stateJson: dco_decode_String(arr[0]),
      action: dco_decode_String(arr[1]),
      reconcile: dco_decode_bool(arr[2]),
      dirtyBooks: dco_decode_list_String(arr[3]),
      phase: dco_decode_String(arr[4]),
      reason: dco_decode_opt_String(arr[5]),
      booksWritten: dco_decode_i_64(arr[6]),
      booksDeleted: dco_decode_i_64(arr[7]),
      keepSocket: dco_decode_bool(arr[8]),
    );
  }

  @protected
  StorageBookDto dco_decode_storage_book_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 10)
      throw Exception('unexpected arr length: expect 10 but see ${arr.length}');
    return StorageBookDto(
      serverId: dco_decode_String(arr[0]),
      bookId: dco_decode_String(arr[1]),
      title: dco_decode_String(arr[2]),
      seriesTitle: dco_decode_String(arr[3]),
      state: dco_decode_String(arr[4]),
      pagesTotal: dco_decode_i_64(arr[5]),
      pagesDone: dco_decode_i_64(arr[6]),
      bytesTotal: dco_decode_i_64(arr[7]),
      bytesDone: dco_decode_i_64(arr[8]),
      onDisk: dco_decode_i_64(arr[9]),
    );
  }

  @protected
  StorageDto dco_decode_storage_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 14)
      throw Exception('unexpected arr length: expect 14 but see ${arr.length}');
    return StorageDto(
      downloadBytes: dco_decode_i_64(arr[0]),
      downloadPageCount: dco_decode_i_64(arr[1]),
      bookCount: dco_decode_i_64(arr[2]),
      perBook: dco_decode_list_storage_book_dto(arr[3]),
      downloadDiskBytes: dco_decode_i_64(arr[4]),
      downloadDiskFiles: dco_decode_i_64(arr[5]),
      unownedBooks: dco_decode_i_64(arr[6]),
      unownedBytes: dco_decode_i_64(arr[7]),
      cachePageBytes: dco_decode_i_64(arr[8]),
      cachePrefetchBytes: dco_decode_i_64(arr[9]),
      cacheThumbnailBytes: dco_decode_i_64(arr[10]),
      cacheTotalBytes: dco_decode_i_64(arr[11]),
      cacheBudgetBytes: dco_decode_i_64(arr[12]),
      freeVolumeBytes: dco_decode_i_64(arr[13]),
    );
  }

  @protected
  TableRows dco_decode_table_rows(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return TableRows(
      table: dco_decode_String(arr[0]),
      rows: dco_decode_i_64(arr[1]),
    );
  }

  @protected
  ThumbnailRow dco_decode_thumbnail_row(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 6)
      throw Exception('unexpected arr length: expect 6 but see ${arr.length}');
    return ThumbnailRow(
      serverId: dco_decode_String(arr[0]),
      remoteId: dco_decode_String(arr[1]),
      variant: dco_decode_String(arr[2]),
      localPath: dco_decode_String(arr[3]),
      sizeBytes: dco_decode_i_64(arr[4]),
      lastAccess: dco_decode_String(arr[5]),
    );
  }

  @protected
  Tombstone dco_decode_tombstone(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return Tombstone(
      serverId: dco_decode_String(arr[0]),
      entityType: dco_decode_String(arr[1]),
      remoteId: dco_decode_String(arr[2]),
      deletedAt: dco_decode_String(arr[3]),
      cause: dco_decode_String(arr[4]),
    );
  }

  @protected
  int dco_decode_u_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  int dco_decode_u_8(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  void dco_decode_unit(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return;
  }

  @protected
  UploadOutcomeDto dco_decode_upload_outcome_dto(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 11)
      throw Exception('unexpected arr length: expect 11 but see ${arr.length}');
    return UploadOutcomeDto(
      serverId: dco_decode_String(arr[0]),
      considered: dco_decode_i_64(arr[1]),
      uploaded: dco_decode_i_64(arr[2]),
      alreadyApplied: dco_decode_i_64(arr[3]),
      remoteWins: dco_decode_i_64(arr[4]),
      gone: dco_decode_i_64(arr[5]),
      retried: dco_decode_i_64(arr[6]),
      rejected: dco_decode_i_64(arr[7]),
      blockedAuthentication: dco_decode_i_64(arr[8]),
      status: dco_decode_String(arr[9]),
      outbox: dco_decode_outbox_status_dto(arr[10]),
    );
  }

  @protected
  BigInt dco_decode_usize(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dcoDecodeU64(raw);
  }

  @protected
  Map<String, String> sse_decode_Map_String_String_None(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_list_record_string_string(deserializer);
    return Map.fromEntries(inner.map((e) => MapEntry(e.$1, e.$2)));
  }

  @protected
  String sse_decode_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_list_prim_u_8_strict(deserializer);
    return utf8.decoder.convert(inner);
  }

  @protected
  ApiPolicyDto sse_decode_api_policy_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_contractVersion = sse_decode_String(deserializer);
    var var_snapshotVersion = sse_decode_String(deserializer);
    var var_minServerVersion = sse_decode_String(deserializer);
    return ApiPolicyDto(
        contractVersion: var_contractVersion,
        snapshotVersion: var_snapshotVersion,
        minServerVersion: var_minServerVersion);
  }

  @protected
  AuthStateDto sse_decode_auth_state_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_state = sse_decode_String(deserializer);
    var var_at = sse_decode_String(deserializer);
    return AuthStateDto(serverId: var_serverId, state: var_state, at: var_at);
  }

  @protected
  AuthType sse_decode_auth_type(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return AuthType.values[inner];
  }

  @protected
  AuthorRow sse_decode_author_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_name = sse_decode_String(deserializer);
    var var_role = sse_decode_String(deserializer);
    return AuthorRow(name: var_name, role: var_role);
  }

  @protected
  BookDetailRow sse_decode_book_detail_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_seriesId = sse_decode_String(deserializer);
    var var_seriesTitle = sse_decode_opt_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_number = sse_decode_opt_String(deserializer);
    var var_numberSort = sse_decode_opt_box_autoadd_f_64(deserializer);
    var var_summary = sse_decode_opt_String(deserializer);
    var var_isbn = sse_decode_opt_String(deserializer);
    var var_releaseDate = sse_decode_opt_String(deserializer);
    var var_mediaType = sse_decode_opt_String(deserializer);
    var var_pagesCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_fileSize = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_createdAt = sse_decode_opt_String(deserializer);
    var var_lastModified = sse_decode_opt_String(deserializer);
    var var_tags = sse_decode_list_String(deserializer);
    var var_authors = sse_decode_list_author_row(deserializer);
    var var_progressPage = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_progressCompleted = sse_decode_bool(deserializer);
    return BookDetailRow(
        serverId: var_serverId,
        remoteId: var_remoteId,
        seriesId: var_seriesId,
        seriesTitle: var_seriesTitle,
        title: var_title,
        number: var_number,
        numberSort: var_numberSort,
        summary: var_summary,
        isbn: var_isbn,
        releaseDate: var_releaseDate,
        mediaType: var_mediaType,
        pagesCount: var_pagesCount,
        fileSize: var_fileSize,
        createdAt: var_createdAt,
        lastModified: var_lastModified,
        tags: var_tags,
        authors: var_authors,
        progressPage: var_progressPage,
        progressCompleted: var_progressCompleted);
  }

  @protected
  BookPageResult sse_decode_book_page_result(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_items = sse_decode_list_book_row(deserializer);
    var var_total = sse_decode_i_64(deserializer);
    return BookPageResult(items: var_items, total: var_total);
  }

  @protected
  BookRow sse_decode_book_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_seriesId = sse_decode_String(deserializer);
    var var_seriesTitle = sse_decode_opt_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_number = sse_decode_opt_String(deserializer);
    var var_numberSort = sse_decode_opt_box_autoadd_f_64(deserializer);
    var var_fileSize = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_mediaType = sse_decode_opt_String(deserializer);
    var var_pagesCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_createdAt = sse_decode_opt_String(deserializer);
    var var_lastModified = sse_decode_opt_String(deserializer);
    var var_progressPage = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_progressCompleted = sse_decode_bool(deserializer);
    var var_ftsRowid = sse_decode_opt_box_autoadd_i_64(deserializer);
    return BookRow(
        serverId: var_serverId,
        remoteId: var_remoteId,
        seriesId: var_seriesId,
        seriesTitle: var_seriesTitle,
        title: var_title,
        number: var_number,
        numberSort: var_numberSort,
        fileSize: var_fileSize,
        mediaType: var_mediaType,
        pagesCount: var_pagesCount,
        createdAt: var_createdAt,
        lastModified: var_lastModified,
        progressPage: var_progressPage,
        progressCompleted: var_progressCompleted,
        ftsRowid: var_ftsRowid);
  }

  @protected
  bool sse_decode_bool(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint8() != 0;
  }

  @protected
  BootstrapSummary sse_decode_bootstrap_summary(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_syncedSeries = sse_decode_usize(deserializer);
    var var_totalElements = sse_decode_i_64(deserializer);
    var var_hasMorePages = sse_decode_bool(deserializer);
    return BootstrapSummary(
        serverId: var_serverId,
        syncedSeries: var_syncedSeries,
        totalElements: var_totalElements,
        hasMorePages: var_hasMorePages);
  }

  @protected
  BookDetailRow sse_decode_box_autoadd_book_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_book_detail_row(deserializer));
  }

  @protected
  bool sse_decode_box_autoadd_bool(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_bool(deserializer));
  }

  @protected
  BuildInfo sse_decode_box_autoadd_build_info(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_build_info(deserializer));
  }

  @protected
  CollectionDetailRow sse_decode_box_autoadd_collection_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_collection_detail_row(deserializer));
  }

  @protected
  DeviceProfileDto sse_decode_box_autoadd_device_profile_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_device_profile_dto(deserializer));
  }

  @protected
  DownloadPumpDto sse_decode_box_autoadd_download_pump_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_download_pump_dto(deserializer));
  }

  @protected
  double sse_decode_box_autoadd_f_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_f_64(deserializer));
  }

  @protected
  GitCommit sse_decode_box_autoadd_git_commit(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_git_commit(deserializer));
  }

  @protected
  GitInfo sse_decode_box_autoadd_git_info(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_git_info(deserializer));
  }

  @protected
  PlatformInt64 sse_decode_box_autoadd_i_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_i_64(deserializer));
  }

  @protected
  LibraryCountRow sse_decode_box_autoadd_library_count_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_library_count_row(deserializer));
  }

  @protected
  ReaderSettingsDto sse_decode_box_autoadd_reader_settings_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_reader_settings_dto(deserializer));
  }

  @protected
  ReadlistDetailRow sse_decode_box_autoadd_readlist_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_readlist_detail_row(deserializer));
  }

  @protected
  SeriesDetailRow sse_decode_box_autoadd_series_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_series_detail_row(deserializer));
  }

  @protected
  ServerProfile sse_decode_box_autoadd_server_profile(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_server_profile(deserializer));
  }

  @protected
  SsePollResult sse_decode_box_autoadd_sse_poll_result(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_sse_poll_result(deserializer));
  }

  @protected
  BuildInfo sse_decode_build_info(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_artifact = sse_decode_opt_String(deserializer);
    var var_name = sse_decode_opt_String(deserializer);
    var var_time = sse_decode_opt_String(deserializer);
    var var_version = sse_decode_opt_String(deserializer);
    var var_group = sse_decode_opt_String(deserializer);
    return BuildInfo(
        artifact: var_artifact,
        name: var_name,
        time: var_time,
        version: var_version,
        group: var_group);
  }

  @protected
  CacheCleanupDto sse_decode_cache_cleanup_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_ghostRows = sse_decode_i_64(deserializer);
    var var_orphanFiles = sse_decode_i_64(deserializer);
    var var_staleParts = sse_decode_i_64(deserializer);
    var var_corrupt = sse_decode_i_64(deserializer);
    var var_kindRepaired = sse_decode_i_64(deserializer);
    var var_evicted = sse_decode_i_64(deserializer);
    var var_freedBytes = sse_decode_i_64(deserializer);
    var var_bytesAfter = sse_decode_i_64(deserializer);
    return CacheCleanupDto(
        ghostRows: var_ghostRows,
        orphanFiles: var_orphanFiles,
        staleParts: var_staleParts,
        corrupt: var_corrupt,
        kindRepaired: var_kindRepaired,
        evicted: var_evicted,
        freedBytes: var_freedBytes,
        bytesAfter: var_bytesAfter);
  }

  @protected
  CacheStatsDto sse_decode_cache_stats_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pageBytes = sse_decode_i_64(deserializer);
    var var_prefetchBytes = sse_decode_i_64(deserializer);
    var var_downloadBytes = sse_decode_i_64(deserializer);
    var var_poolBudgetBytes = sse_decode_i_64(deserializer);
    var var_memoryBytes = sse_decode_i_64(deserializer);
    var var_memoryPeakBytes = sse_decode_i_64(deserializer);
    var var_memoryEntries = sse_decode_i_64(deserializer);
    var var_memoryHits = sse_decode_i_64(deserializer);
    var var_memoryMisses = sse_decode_i_64(deserializer);
    var var_memoryEvictions = sse_decode_i_64(deserializer);
    var var_memoryRefused = sse_decode_i_64(deserializer);
    var var_diskBytes = sse_decode_i_64(deserializer);
    var var_ledgerBytes = sse_decode_i_64(deserializer);
    var var_openReaders = sse_decode_i_64(deserializer);
    return CacheStatsDto(
        pageBytes: var_pageBytes,
        prefetchBytes: var_prefetchBytes,
        downloadBytes: var_downloadBytes,
        poolBudgetBytes: var_poolBudgetBytes,
        memoryBytes: var_memoryBytes,
        memoryPeakBytes: var_memoryPeakBytes,
        memoryEntries: var_memoryEntries,
        memoryHits: var_memoryHits,
        memoryMisses: var_memoryMisses,
        memoryEvictions: var_memoryEvictions,
        memoryRefused: var_memoryRefused,
        diskBytes: var_diskBytes,
        ledgerBytes: var_ledgerBytes,
        openReaders: var_openReaders);
  }

  @protected
  CollectionDetailRow sse_decode_collection_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_remoteId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_ordered = sse_decode_bool(deserializer);
    var var_filtered = sse_decode_bool(deserializer);
    var var_createdDate = sse_decode_opt_String(deserializer);
    var var_lastModifiedDate = sse_decode_opt_String(deserializer);
    var var_members = sse_decode_series_page_result(deserializer);
    return CollectionDetailRow(
        remoteId: var_remoteId,
        name: var_name,
        ordered: var_ordered,
        filtered: var_filtered,
        createdDate: var_createdDate,
        lastModifiedDate: var_lastModifiedDate,
        members: var_members);
  }

  @protected
  CollectionPageResult sse_decode_collection_page_result(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_items = sse_decode_list_collection_row(deserializer);
    var var_total = sse_decode_i_64(deserializer);
    return CollectionPageResult(items: var_items, total: var_total);
  }

  @protected
  CollectionRef sse_decode_collection_ref(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_remoteId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    return CollectionRef(remoteId: var_remoteId, name: var_name);
  }

  @protected
  CollectionRow sse_decode_collection_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_ordered = sse_decode_bool(deserializer);
    var var_filtered = sse_decode_bool(deserializer);
    var var_createdDate = sse_decode_opt_String(deserializer);
    var var_lastModifiedDate = sse_decode_opt_String(deserializer);
    return CollectionRow(
        serverId: var_serverId,
        remoteId: var_remoteId,
        name: var_name,
        ordered: var_ordered,
        filtered: var_filtered,
        createdDate: var_createdDate,
        lastModifiedDate: var_lastModifiedDate);
  }

  @protected
  ConnectionResult sse_decode_connection_result(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverInfo = sse_decode_server_info(deserializer);
    var var_serverVersion = sse_decode_opt_String(deserializer);
    var var_libraries = sse_decode_list_library(deserializer);
    var var_capabilities = sse_decode_list_String(deserializer);
    return ConnectionResult(
        serverInfo: var_serverInfo,
        serverVersion: var_serverVersion,
        libraries: var_libraries,
        capabilities: var_capabilities);
  }

  @protected
  ContinueReadingRow sse_decode_continue_reading_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_bookId = sse_decode_String(deserializer);
    var var_bookTitle = sse_decode_String(deserializer);
    var var_number = sse_decode_opt_String(deserializer);
    var var_seriesId = sse_decode_String(deserializer);
    var var_seriesName = sse_decode_String(deserializer);
    var var_page = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_totalPages = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_progressPct = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_localUpdatedAt = sse_decode_opt_String(deserializer);
    return ContinueReadingRow(
        bookId: var_bookId,
        bookTitle: var_bookTitle,
        number: var_number,
        seriesId: var_seriesId,
        seriesName: var_seriesName,
        page: var_page,
        totalPages: var_totalPages,
        progressPct: var_progressPct,
        localUpdatedAt: var_localUpdatedAt);
  }

  @protected
  CoreError sse_decode_core_error(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_code = sse_decode_error_code(deserializer);
    var var_message = sse_decode_String(deserializer);
    var var_retryable = sse_decode_bool(deserializer);
    var var_needsUser = sse_decode_bool(deserializer);
    return CoreError(
        code: var_code,
        message: var_message,
        retryable: var_retryable,
        needsUser: var_needsUser);
  }

  @protected
  DbHealth sse_decode_db_health(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_schemaVersion = sse_decode_i_64(deserializer);
    var var_integrity = sse_decode_String(deserializer);
    var var_journalMode = sse_decode_String(deserializer);
    var var_pageSize = sse_decode_i_64(deserializer);
    var var_pageCount = sse_decode_i_64(deserializer);
    var var_freelistCount = sse_decode_i_64(deserializer);
    var var_busyTimeoutMs = sse_decode_i_64(deserializer);
    var var_foreignKeysOn = sse_decode_bool(deserializer);
    var var_fileBytes = sse_decode_i_64(deserializer);
    var var_tables = sse_decode_list_table_rows(deserializer);
    return DbHealth(
        schemaVersion: var_schemaVersion,
        integrity: var_integrity,
        journalMode: var_journalMode,
        pageSize: var_pageSize,
        pageCount: var_pageCount,
        freelistCount: var_freelistCount,
        busyTimeoutMs: var_busyTimeoutMs,
        foreignKeysOn: var_foreignKeysOn,
        fileBytes: var_fileBytes,
        tables: var_tables);
  }

  @protected
  DeviceProfileDto sse_decode_device_profile_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_deviceMemoryBytes = sse_decode_i_64(deserializer);
    var var_cacheBudgetBytes = sse_decode_i_64(deserializer);
    var var_avgPageBytesHint = sse_decode_i_64(deserializer);
    var var_decodedPageBytes = sse_decode_i_64(deserializer);
    var var_network = sse_decode_String(deserializer);
    var var_stable = sse_decode_bool(deserializer);
    return DeviceProfileDto(
        deviceMemoryBytes: var_deviceMemoryBytes,
        cacheBudgetBytes: var_cacheBudgetBytes,
        avgPageBytesHint: var_avgPageBytesHint,
        decodedPageBytes: var_decodedPageBytes,
        network: var_network,
        stable: var_stable);
  }

  @protected
  DiagnosticsDto sse_decode_diagnostics_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_db = sse_decode_db_health(deserializer);
    var var_auth = sse_decode_auth_state_dto(deserializer);
    var var_outboxQueuedRows = sse_decode_i_64(deserializer);
    var var_sync_ = sse_decode_list_entity_sync_state(deserializer);
    var var_outbox = sse_decode_outbox_status_dto(deserializer);
    var var_cache = sse_decode_cache_stats_dto(deserializer);
    var var_storage = sse_decode_storage_dto(deserializer);
    var var_queue = sse_decode_list_queue_state_count_dto(deserializer);
    var var_policy = sse_decode_api_policy_dto(deserializer);
    var var_log = sse_decode_log_stats(deserializer);
    return DiagnosticsDto(
        serverId: var_serverId,
        db: var_db,
        auth: var_auth,
        outboxQueuedRows: var_outboxQueuedRows,
        sync_: var_sync_,
        outbox: var_outbox,
        cache: var_cache,
        storage: var_storage,
        queue: var_queue,
        policy: var_policy,
        log: var_log);
  }

  @protected
  DownloadBookDto sse_decode_download_book_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_bookId = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_seriesTitle = sse_decode_String(deserializer);
    var var_state = sse_decode_String(deserializer);
    var var_pagesTotal = sse_decode_i_64(deserializer);
    var var_pagesDone = sse_decode_i_64(deserializer);
    var var_bytesTotal = sse_decode_i_64(deserializer);
    var var_bytesDone = sse_decode_i_64(deserializer);
    var var_position = sse_decode_i_64(deserializer);
    var var_lastError = sse_decode_String(deserializer);
    var var_nextRetryAt = sse_decode_String(deserializer);
    var var_remoteLastModified = sse_decode_String(deserializer);
    var var_allowCellular = sse_decode_bool(deserializer);
    var var_stale = sse_decode_bool(deserializer);
    return DownloadBookDto(
        serverId: var_serverId,
        bookId: var_bookId,
        title: var_title,
        seriesTitle: var_seriesTitle,
        state: var_state,
        pagesTotal: var_pagesTotal,
        pagesDone: var_pagesDone,
        bytesTotal: var_bytesTotal,
        bytesDone: var_bytesDone,
        position: var_position,
        lastError: var_lastError,
        nextRetryAt: var_nextRetryAt,
        remoteLastModified: var_remoteLastModified,
        allowCellular: var_allowCellular,
        stale: var_stale);
  }

  @protected
  DownloadDeleteDto sse_decode_download_delete_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_books = sse_decode_i_64(deserializer);
    var var_files = sse_decode_i_64(deserializer);
    var var_freedBytes = sse_decode_i_64(deserializer);
    return DownloadDeleteDto(
        books: var_books, files: var_files, freedBytes: var_freedBytes);
  }

  @protected
  DownloadPumpDto sse_decode_download_pump_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_book = sse_decode_String(deserializer);
    var var_state = sse_decode_String(deserializer);
    var var_served = sse_decode_i_64(deserializer);
    var var_failedPages = sse_decode_i_64(deserializer);
    var var_bytesWritten = sse_decode_i_64(deserializer);
    var var_pagesDone = sse_decode_i_64(deserializer);
    var var_pagesTotal = sse_decode_i_64(deserializer);
    var var_stopReason = sse_decode_String(deserializer);
    var var_nextInMs = sse_decode_i_64(deserializer);
    var var_pumpMs = sse_decode_i_64(deserializer);
    var var_lastError = sse_decode_String(deserializer);
    var var_repairs = sse_decode_i_64(deserializer);
    var var_partsSwept = sse_decode_i_64(deserializer);
    var var_adopted = sse_decode_i_64(deserializer);
    var var_ghostRows = sse_decode_i_64(deserializer);
    var var_queueActive = sse_decode_bool(deserializer);
    return DownloadPumpDto(
        book: var_book,
        state: var_state,
        served: var_served,
        failedPages: var_failedPages,
        bytesWritten: var_bytesWritten,
        pagesDone: var_pagesDone,
        pagesTotal: var_pagesTotal,
        stopReason: var_stopReason,
        nextInMs: var_nextInMs,
        pumpMs: var_pumpMs,
        lastError: var_lastError,
        repairs: var_repairs,
        partsSwept: var_partsSwept,
        adopted: var_adopted,
        ghostRows: var_ghostRows,
        queueActive: var_queueActive);
  }

  @protected
  DownloadSweepDto sse_decode_download_sweep_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_books = sse_decode_i_64(deserializer);
    var var_staleParts = sse_decode_i_64(deserializer);
    var var_ghostRows = sse_decode_i_64(deserializer);
    var var_corrupt = sse_decode_i_64(deserializer);
    var var_sizeMismatch = sse_decode_i_64(deserializer);
    var var_adoptedFiles = sse_decode_i_64(deserializer);
    var var_countersRepaired = sse_decode_i_64(deserializer);
    var var_manifestsRewritten = sse_decode_i_64(deserializer);
    var var_pagesRemoved = sse_decode_i_64(deserializer);
    var var_unownedBooks = sse_decode_i_64(deserializer);
    var var_unownedBytes = sse_decode_i_64(deserializer);
    var var_freedBytes = sse_decode_i_64(deserializer);
    return DownloadSweepDto(
        books: var_books,
        staleParts: var_staleParts,
        ghostRows: var_ghostRows,
        corrupt: var_corrupt,
        sizeMismatch: var_sizeMismatch,
        adoptedFiles: var_adoptedFiles,
        countersRepaired: var_countersRepaired,
        manifestsRewritten: var_manifestsRewritten,
        pagesRemoved: var_pagesRemoved,
        unownedBooks: var_unownedBooks,
        unownedBytes: var_unownedBytes,
        freedBytes: var_freedBytes);
  }

  @protected
  EntitySyncState sse_decode_entity_sync_state(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_entityType = sse_decode_String(deserializer);
    var var_lastSyncAt = sse_decode_opt_String(deserializer);
    var var_syncCursor = sse_decode_opt_String(deserializer);
    var var_syncStatus = sse_decode_String(deserializer);
    var var_lastError = sse_decode_opt_String(deserializer);
    var var_lastFullSync = sse_decode_opt_String(deserializer);
    var var_lastSuccessfulSync = sse_decode_opt_String(deserializer);
    return EntitySyncState(
        serverId: var_serverId,
        entityType: var_entityType,
        lastSyncAt: var_lastSyncAt,
        syncCursor: var_syncCursor,
        syncStatus: var_syncStatus,
        lastError: var_lastError,
        lastFullSync: var_lastFullSync,
        lastSuccessfulSync: var_lastSuccessfulSync);
  }

  @protected
  ErrorCode sse_decode_error_code(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return ErrorCode.values[inner];
  }

  @protected
  double sse_decode_f_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getFloat64();
  }

  @protected
  FilterOptions sse_decode_filter_options(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_tags = sse_decode_list_String(deserializer);
    var var_genres = sse_decode_list_String(deserializer);
    var var_statuses = sse_decode_list_String(deserializer);
    return FilterOptions(
        tags: var_tags, genres: var_genres, statuses: var_statuses);
  }

  @protected
  FullSyncSummary sse_decode_full_sync_summary(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_libraries = sse_decode_usize(deserializer);
    var var_series = sse_decode_usize(deserializer);
    var var_books = sse_decode_usize(deserializer);
    var var_collections = sse_decode_usize(deserializer);
    var var_readlists = sse_decode_usize(deserializer);
    var var_readProgress = sse_decode_usize(deserializer);
    var var_seriesPages = sse_decode_u_32(deserializer);
    var var_bookPages = sse_decode_u_32(deserializer);
    var var_skippedSteps = sse_decode_list_String(deserializer);
    var var_resumedSteps = sse_decode_list_String(deserializer);
    return FullSyncSummary(
        serverId: var_serverId,
        libraries: var_libraries,
        series: var_series,
        books: var_books,
        collections: var_collections,
        readlists: var_readlists,
        readProgress: var_readProgress,
        seriesPages: var_seriesPages,
        bookPages: var_bookPages,
        skippedSteps: var_skippedSteps,
        resumedSteps: var_resumedSteps);
  }

  @protected
  GitCommit sse_decode_git_commit(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_opt_String(deserializer);
    var var_time = sse_decode_opt_String(deserializer);
    return GitCommit(id: var_id, time: var_time);
  }

  @protected
  GitInfo sse_decode_git_info(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_branch = sse_decode_opt_String(deserializer);
    var var_commit = sse_decode_opt_box_autoadd_git_commit(deserializer);
    return GitInfo(branch: var_branch, commit: var_commit);
  }

  @protected
  int sse_decode_i_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getInt32();
  }

  @protected
  PlatformInt64 sse_decode_i_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getPlatformInt64();
  }

  @protected
  Library sse_decode_library(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_root = sse_decode_String(deserializer);
    var var_unavailable = sse_decode_opt_box_autoadd_bool(deserializer);
    return Library(
        id: var_id,
        name: var_name,
        root: var_root,
        unavailable: var_unavailable);
  }

  @protected
  LibraryCountRow sse_decode_library_count_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_remoteId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_root = sse_decode_opt_String(deserializer);
    var var_unavailable = sse_decode_bool(deserializer);
    var var_seriesCount = sse_decode_i_64(deserializer);
    var var_bookCount = sse_decode_i_64(deserializer);
    var var_readCount = sse_decode_i_64(deserializer);
    return LibraryCountRow(
        remoteId: var_remoteId,
        name: var_name,
        root: var_root,
        unavailable: var_unavailable,
        seriesCount: var_seriesCount,
        bookCount: var_bookCount,
        readCount: var_readCount);
  }

  @protected
  List<String> sse_decode_list_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <String>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_String(deserializer));
    }
    return ans_;
  }

  @protected
  List<AuthorRow> sse_decode_list_author_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <AuthorRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_author_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<BookRow> sse_decode_list_book_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <BookRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_book_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<CollectionRef> sse_decode_list_collection_ref(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <CollectionRef>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_collection_ref(deserializer));
    }
    return ans_;
  }

  @protected
  List<CollectionRow> sse_decode_list_collection_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <CollectionRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_collection_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<ContinueReadingRow> sse_decode_list_continue_reading_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ContinueReadingRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_continue_reading_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<DownloadBookDto> sse_decode_list_download_book_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <DownloadBookDto>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_download_book_dto(deserializer));
    }
    return ans_;
  }

  @protected
  List<EntitySyncState> sse_decode_list_entity_sync_state(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <EntitySyncState>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_entity_sync_state(deserializer));
    }
    return ans_;
  }

  @protected
  List<Library> sse_decode_list_library(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <Library>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_library(deserializer));
    }
    return ans_;
  }

  @protected
  List<LibraryCountRow> sse_decode_list_library_count_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <LibraryCountRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_library_count_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<Uint32List> sse_decode_list_list_prim_u_32_strict(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <Uint32List>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_list_prim_u_32_strict(deserializer));
    }
    return ans_;
  }

  @protected
  List<LogRecord> sse_decode_list_log_record(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <LogRecord>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_log_record(deserializer));
    }
    return ans_;
  }

  @protected
  List<OutboxEntryDto> sse_decode_list_outbox_entry_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <OutboxEntryDto>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_outbox_entry_dto(deserializer));
    }
    return ans_;
  }

  @protected
  Uint32List sse_decode_list_prim_u_32_strict(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var len_ = sse_decode_i_32(deserializer);
    return deserializer.buffer.getUint32List(len_);
  }

  @protected
  Uint8List sse_decode_list_prim_u_8_strict(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var len_ = sse_decode_i_32(deserializer);
    return deserializer.buffer.getUint8List(len_);
  }

  @protected
  List<QueueStateCountDto> sse_decode_list_queue_state_count_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <QueueStateCountDto>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_queue_state_count_dto(deserializer));
    }
    return ans_;
  }

  @protected
  List<ReadlistRow> sse_decode_list_readlist_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ReadlistRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_readlist_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<(String, String)> sse_decode_list_record_string_string(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <(String, String)>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_record_string_string(deserializer));
    }
    return ans_;
  }

  @protected
  List<SeriesRow> sse_decode_list_series_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <SeriesRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_series_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<ServerProfile> sse_decode_list_server_profile(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ServerProfile>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_server_profile(deserializer));
    }
    return ans_;
  }

  @protected
  List<StorageBookDto> sse_decode_list_storage_book_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <StorageBookDto>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_storage_book_dto(deserializer));
    }
    return ans_;
  }

  @protected
  List<TableRows> sse_decode_list_table_rows(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <TableRows>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_table_rows(deserializer));
    }
    return ans_;
  }

  @protected
  List<ThumbnailRow> sse_decode_list_thumbnail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ThumbnailRow>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_thumbnail_row(deserializer));
    }
    return ans_;
  }

  @protected
  List<Tombstone> sse_decode_list_tombstone(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <Tombstone>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_tombstone(deserializer));
    }
    return ans_;
  }

  @protected
  LogRecord sse_decode_log_record(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_level = sse_decode_String(deserializer);
    var var_target = sse_decode_String(deserializer);
    var var_message = sse_decode_String(deserializer);
    var var_at = sse_decode_String(deserializer);
    return LogRecord(
        level: var_level, target: var_target, message: var_message, at: var_at);
  }

  @protected
  LogStats sse_decode_log_stats(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_installed = sse_decode_bool(deserializer);
    var var_capacity = sse_decode_i_64(deserializer);
    var var_retained = sse_decode_i_64(deserializer);
    var var_dropped = sse_decode_i_64(deserializer);
    var var_errors = sse_decode_i_64(deserializer);
    var var_warnings = sse_decode_i_64(deserializer);
    var var_info = sse_decode_i_64(deserializer);
    var var_debug = sse_decode_i_64(deserializer);
    var var_trace = sse_decode_i_64(deserializer);
    var var_lastError = sse_decode_opt_String(deserializer);
    var var_maxLevel = sse_decode_String(deserializer);
    return LogStats(
        installed: var_installed,
        capacity: var_capacity,
        retained: var_retained,
        dropped: var_dropped,
        errors: var_errors,
        warnings: var_warnings,
        info: var_info,
        debug: var_debug,
        trace: var_trace,
        lastError: var_lastError,
        maxLevel: var_maxLevel);
  }

  @protected
  String? sse_decode_opt_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_String(deserializer));
    } else {
      return null;
    }
  }

  @protected
  BookDetailRow? sse_decode_opt_box_autoadd_book_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_book_detail_row(deserializer));
    } else {
      return null;
    }
  }

  @protected
  bool? sse_decode_opt_box_autoadd_bool(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_bool(deserializer));
    } else {
      return null;
    }
  }

  @protected
  BuildInfo? sse_decode_opt_box_autoadd_build_info(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_build_info(deserializer));
    } else {
      return null;
    }
  }

  @protected
  CollectionDetailRow? sse_decode_opt_box_autoadd_collection_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_collection_detail_row(deserializer));
    } else {
      return null;
    }
  }

  @protected
  DownloadPumpDto? sse_decode_opt_box_autoadd_download_pump_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_download_pump_dto(deserializer));
    } else {
      return null;
    }
  }

  @protected
  double? sse_decode_opt_box_autoadd_f_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_f_64(deserializer));
    } else {
      return null;
    }
  }

  @protected
  GitCommit? sse_decode_opt_box_autoadd_git_commit(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_git_commit(deserializer));
    } else {
      return null;
    }
  }

  @protected
  GitInfo? sse_decode_opt_box_autoadd_git_info(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_git_info(deserializer));
    } else {
      return null;
    }
  }

  @protected
  PlatformInt64? sse_decode_opt_box_autoadd_i_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_i_64(deserializer));
    } else {
      return null;
    }
  }

  @protected
  LibraryCountRow? sse_decode_opt_box_autoadd_library_count_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_library_count_row(deserializer));
    } else {
      return null;
    }
  }

  @protected
  ReadlistDetailRow? sse_decode_opt_box_autoadd_readlist_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_readlist_detail_row(deserializer));
    } else {
      return null;
    }
  }

  @protected
  SeriesDetailRow? sse_decode_opt_box_autoadd_series_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_series_detail_row(deserializer));
    } else {
      return null;
    }
  }

  @protected
  ServerProfile? sse_decode_opt_box_autoadd_server_profile(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_server_profile(deserializer));
    } else {
      return null;
    }
  }

  @protected
  SsePollResult? sse_decode_opt_box_autoadd_sse_poll_result(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_sse_poll_result(deserializer));
    } else {
      return null;
    }
  }

  @protected
  OutboxEntryDto sse_decode_outbox_entry_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_entityId = sse_decode_String(deserializer);
    var var_mutationType = sse_decode_String(deserializer);
    var var_retryCount = sse_decode_i_64(deserializer);
    var var_lastError = sse_decode_opt_String(deserializer);
    var var_state = sse_decode_String(deserializer);
    var var_nextRetryAt = sse_decode_opt_String(deserializer);
    var var_createdAt = sse_decode_String(deserializer);
    return OutboxEntryDto(
        id: var_id,
        entityId: var_entityId,
        mutationType: var_mutationType,
        retryCount: var_retryCount,
        lastError: var_lastError,
        state: var_state,
        nextRetryAt: var_nextRetryAt,
        createdAt: var_createdAt);
  }

  @protected
  OutboxStatusDto sse_decode_outbox_status_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_pending = sse_decode_i_64(deserializer);
    var var_waiting = sse_decode_i_64(deserializer);
    var var_failed = sse_decode_i_64(deserializer);
    var var_total = sse_decode_i_64(deserializer);
    var var_failedEntries = sse_decode_list_outbox_entry_dto(deserializer);
    return OutboxStatusDto(
        serverId: var_serverId,
        pending: var_pending,
        waiting: var_waiting,
        failed: var_failed,
        total: var_total,
        failedEntries: var_failedEntries);
  }

  @protected
  QueueStateCountDto sse_decode_queue_state_count_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_state = sse_decode_String(deserializer);
    var var_books = sse_decode_i_64(deserializer);
    var var_pagesDone = sse_decode_i_64(deserializer);
    var var_pagesTotal = sse_decode_i_64(deserializer);
    var var_bytesDone = sse_decode_i_64(deserializer);
    var var_bytesTotal = sse_decode_i_64(deserializer);
    return QueueStateCountDto(
        state: var_state,
        books: var_books,
        pagesDone: var_pagesDone,
        pagesTotal: var_pagesTotal,
        bytesDone: var_bytesDone,
        bytesTotal: var_bytesTotal);
  }

  @protected
  ReadTargetRow sse_decode_read_target_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_book = sse_decode_book_row(deserializer);
    var var_intent = sse_decode_String(deserializer);
    var var_position = sse_decode_i_64(deserializer);
    var var_bookCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_complete = sse_decode_bool(deserializer);
    return ReadTargetRow(
        book: var_book,
        intent: var_intent,
        position: var_position,
        bookCount: var_bookCount,
        complete: var_complete);
  }

  @protected
  ReaderBookDto sse_decode_reader_book_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_bookId = sse_decode_String(deserializer);
    var var_pageCount = sse_decode_i_64(deserializer);
    var var_paged = sse_decode_bool(deserializer);
    var var_reflowable = sse_decode_bool(deserializer);
    var var_fallback = sse_decode_opt_String(deserializer);
    var var_fromMirror = sse_decode_bool(deserializer);
    var var_startPage = sse_decode_i_64(deserializer);
    var var_startPageOffsetRatio =
        sse_decode_opt_box_autoadd_f_64(deserializer);
    var var_layout = sse_decode_reader_layout_dto(deserializer);
    return ReaderBookDto(
        serverId: var_serverId,
        bookId: var_bookId,
        pageCount: var_pageCount,
        paged: var_paged,
        reflowable: var_reflowable,
        fallback: var_fallback,
        fromMirror: var_fromMirror,
        startPage: var_startPage,
        startPageOffsetRatio: var_startPageOffsetRatio,
        layout: var_layout);
  }

  @protected
  ReaderLayoutDto sse_decode_reader_layout_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_spreads = sse_decode_list_list_prim_u_32_strict(deserializer);
    var var_spread = sse_decode_i_64(deserializer);
    var var_page = sse_decode_i_64(deserializer);
    var var_axis = sse_decode_String(deserializer);
    var var_reversed = sse_decode_bool(deserializer);
    var var_advanceSwipe = sse_decode_String(deserializer);
    var var_retreatSwipe = sse_decode_String(deserializer);
    var var_tapNext = sse_decode_String(deserializer);
    var var_tapPrev = sse_decode_String(deserializer);
    var var_mode = sse_decode_String(deserializer);
    var var_direction = sse_decode_String(deserializer);
    var var_pageGap = sse_decode_i_64(deserializer);
    var var_background = sse_decode_String(deserializer);
    return ReaderLayoutDto(
        spreads: var_spreads,
        spread: var_spread,
        page: var_page,
        axis: var_axis,
        reversed: var_reversed,
        advanceSwipe: var_advanceSwipe,
        retreatSwipe: var_retreatSwipe,
        tapNext: var_tapNext,
        tapPrev: var_tapPrev,
        mode: var_mode,
        direction: var_direction,
        pageGap: var_pageGap,
        background: var_background);
  }

  @protected
  ReaderSettingsDto sse_decode_reader_settings_dto(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_mode = sse_decode_String(deserializer);
    var var_direction = sse_decode_String(deserializer);
    var var_firstPageSingle = sse_decode_bool(deserializer);
    var var_pageGap = sse_decode_i_64(deserializer);
    var var_background = sse_decode_String(deserializer);
    var var_keepScreenAwake = sse_decode_bool(deserializer);
    var var_brightness = sse_decode_opt_box_autoadd_f_64(deserializer);
    var var_restorePosition = sse_decode_bool(deserializer);
    var var_prefetchForward = sse_decode_i_64(deserializer);
    var var_prefetchBack = sse_decode_i_64(deserializer);
    var var_prefetchCap = sse_decode_i_64(deserializer);
    var var_volumeKeysEnabled = sse_decode_bool(deserializer);
    return ReaderSettingsDto(
        mode: var_mode,
        direction: var_direction,
        firstPageSingle: var_firstPageSingle,
        pageGap: var_pageGap,
        background: var_background,
        keepScreenAwake: var_keepScreenAwake,
        brightness: var_brightness,
        restorePosition: var_restorePosition,
        prefetchForward: var_prefetchForward,
        prefetchBack: var_prefetchBack,
        prefetchCap: var_prefetchCap,
        volumeKeysEnabled: var_volumeKeysEnabled);
  }

  @protected
  ReaderTurnDto sse_decode_reader_turn_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_page = sse_decode_i_64(deserializer);
    var var_spread = sse_decode_i_64(deserializer);
    var var_uploadNow = sse_decode_bool(deserializer);
    return ReaderTurnDto(
        page: var_page, spread: var_spread, uploadNow: var_uploadNow);
  }

  @protected
  ReaderWindowDto sse_decode_reader_window_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_forward = sse_decode_i_64(deserializer);
    var var_back = sse_decode_i_64(deserializer);
    var var_cap = sse_decode_i_64(deserializer);
    var var_memoryBudgetBytes = sse_decode_i_64(deserializer);
    var var_inFlight = sse_decode_i_64(deserializer);
    var var_decodeSlots = sse_decode_i_64(deserializer);
    var var_avgPageBytes = sse_decode_i_64(deserializer);
    var var_pagesPerSpread = sse_decode_i_64(deserializer);
    var var_poolBudgetBytes = sse_decode_i_64(deserializer);
    var var_sweptFreedBytes = sse_decode_i_64(deserializer);
    var var_sweptCorrupt = sse_decode_i_64(deserializer);
    return ReaderWindowDto(
        forward: var_forward,
        back: var_back,
        cap: var_cap,
        memoryBudgetBytes: var_memoryBudgetBytes,
        inFlight: var_inFlight,
        decodeSlots: var_decodeSlots,
        avgPageBytes: var_avgPageBytes,
        pagesPerSpread: var_pagesPerSpread,
        poolBudgetBytes: var_poolBudgetBytes,
        sweptFreedBytes: var_sweptFreedBytes,
        sweptCorrupt: var_sweptCorrupt);
  }

  @protected
  ReadlistDetailRow sse_decode_readlist_detail_row(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_remoteId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_summary = sse_decode_opt_String(deserializer);
    var var_ordered = sse_decode_bool(deserializer);
    var var_filtered = sse_decode_bool(deserializer);
    var var_createdDate = sse_decode_opt_String(deserializer);
    var var_lastModifiedDate = sse_decode_opt_String(deserializer);
    var var_books = sse_decode_book_page_result(deserializer);
    return ReadlistDetailRow(
        remoteId: var_remoteId,
        name: var_name,
        summary: var_summary,
        ordered: var_ordered,
        filtered: var_filtered,
        createdDate: var_createdDate,
        lastModifiedDate: var_lastModifiedDate,
        books: var_books);
  }

  @protected
  ReadlistPageResult sse_decode_readlist_page_result(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_items = sse_decode_list_readlist_row(deserializer);
    var var_total = sse_decode_i_64(deserializer);
    return ReadlistPageResult(items: var_items, total: var_total);
  }

  @protected
  ReadlistRow sse_decode_readlist_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_summary = sse_decode_opt_String(deserializer);
    var var_ordered = sse_decode_bool(deserializer);
    var var_filtered = sse_decode_bool(deserializer);
    var var_createdDate = sse_decode_opt_String(deserializer);
    var var_lastModifiedDate = sse_decode_opt_String(deserializer);
    return ReadlistRow(
        serverId: var_serverId,
        remoteId: var_remoteId,
        name: var_name,
        summary: var_summary,
        ordered: var_ordered,
        filtered: var_filtered,
        createdDate: var_createdDate,
        lastModifiedDate: var_lastModifiedDate);
  }

  @protected
  ReconcileSummary sse_decode_reconcile_summary(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_trigger = sse_decode_String(deserializer);
    var var_seriesUpserted = sse_decode_usize(deserializer);
    var var_seriesAdded = sse_decode_usize(deserializer);
    var var_seriesChanged = sse_decode_usize(deserializer);
    var var_seriesRemoved = sse_decode_usize(deserializer);
    var var_booksUpserted = sse_decode_usize(deserializer);
    var var_booksAdded = sse_decode_usize(deserializer);
    var var_booksChanged = sse_decode_usize(deserializer);
    var var_booksRemoved = sse_decode_usize(deserializer);
    var var_collectionsUpserted = sse_decode_usize(deserializer);
    var var_collectionsAdded = sse_decode_usize(deserializer);
    var var_collectionsChanged = sse_decode_usize(deserializer);
    var var_collectionsRemoved = sse_decode_usize(deserializer);
    var var_readlistsUpserted = sse_decode_usize(deserializer);
    var var_readlistsAdded = sse_decode_usize(deserializer);
    var var_readlistsChanged = sse_decode_usize(deserializer);
    var var_readlistsRemoved = sse_decode_usize(deserializer);
    var var_librariesUpserted = sse_decode_usize(deserializer);
    var var_librariesRemoved = sse_decode_usize(deserializer);
    var var_readProgress = sse_decode_usize(deserializer);
    var var_pagesSwept = sse_decode_u_32(deserializer);
    var var_orphanedCovers = sse_decode_list_String(deserializer);
    var var_clean = sse_decode_bool(deserializer);
    return ReconcileSummary(
        serverId: var_serverId,
        trigger: var_trigger,
        seriesUpserted: var_seriesUpserted,
        seriesAdded: var_seriesAdded,
        seriesChanged: var_seriesChanged,
        seriesRemoved: var_seriesRemoved,
        booksUpserted: var_booksUpserted,
        booksAdded: var_booksAdded,
        booksChanged: var_booksChanged,
        booksRemoved: var_booksRemoved,
        collectionsUpserted: var_collectionsUpserted,
        collectionsAdded: var_collectionsAdded,
        collectionsChanged: var_collectionsChanged,
        collectionsRemoved: var_collectionsRemoved,
        readlistsUpserted: var_readlistsUpserted,
        readlistsAdded: var_readlistsAdded,
        readlistsChanged: var_readlistsChanged,
        readlistsRemoved: var_readlistsRemoved,
        librariesUpserted: var_librariesUpserted,
        librariesRemoved: var_librariesRemoved,
        readProgress: var_readProgress,
        pagesSwept: var_pagesSwept,
        orphanedCovers: var_orphanedCovers,
        clean: var_clean);
  }

  @protected
  (String, String) sse_decode_record_string_string(
      SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_field0 = sse_decode_String(deserializer);
    var var_field1 = sse_decode_String(deserializer);
    return (var_field0, var_field1);
  }

  @protected
  SeriesDetailRow sse_decode_series_detail_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_libraryId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_sortName = sse_decode_opt_String(deserializer);
    var var_status = sse_decode_opt_String(deserializer);
    var var_createdAt = sse_decode_opt_String(deserializer);
    var var_lastModified = sse_decode_opt_String(deserializer);
    var var_booksCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_booksReadCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_booksUnreadCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_booksInProgressCount =
        sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_summary = sse_decode_opt_String(deserializer);
    var var_publisher = sse_decode_opt_String(deserializer);
    var var_readingDirection = sse_decode_opt_String(deserializer);
    var var_language = sse_decode_opt_String(deserializer);
    var var_ageRating = sse_decode_opt_String(deserializer);
    var var_totalBookCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_genres = sse_decode_list_String(deserializer);
    var var_tags = sse_decode_list_String(deserializer);
    var var_authors = sse_decode_list_author_row(deserializer);
    var var_collections = sse_decode_list_collection_ref(deserializer);
    return SeriesDetailRow(
        serverId: var_serverId,
        remoteId: var_remoteId,
        libraryId: var_libraryId,
        name: var_name,
        sortName: var_sortName,
        status: var_status,
        createdAt: var_createdAt,
        lastModified: var_lastModified,
        booksCount: var_booksCount,
        booksReadCount: var_booksReadCount,
        booksUnreadCount: var_booksUnreadCount,
        booksInProgressCount: var_booksInProgressCount,
        summary: var_summary,
        publisher: var_publisher,
        readingDirection: var_readingDirection,
        language: var_language,
        ageRating: var_ageRating,
        totalBookCount: var_totalBookCount,
        genres: var_genres,
        tags: var_tags,
        authors: var_authors,
        collections: var_collections);
  }

  @protected
  SeriesPageResult sse_decode_series_page_result(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_items = sse_decode_list_series_row(deserializer);
    var var_total = sse_decode_i_64(deserializer);
    return SeriesPageResult(items: var_items, total: var_total);
  }

  @protected
  SeriesRow sse_decode_series_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_libraryId = sse_decode_String(deserializer);
    var var_name = sse_decode_String(deserializer);
    var var_sortName = sse_decode_opt_String(deserializer);
    var var_status = sse_decode_opt_String(deserializer);
    var var_createdAt = sse_decode_opt_String(deserializer);
    var var_lastModified = sse_decode_opt_String(deserializer);
    var var_booksCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_booksReadCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_booksUnreadCount = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_booksInProgressCount =
        sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_ftsRowid = sse_decode_opt_box_autoadd_i_64(deserializer);
    return SeriesRow(
        serverId: var_serverId,
        remoteId: var_remoteId,
        libraryId: var_libraryId,
        name: var_name,
        sortName: var_sortName,
        status: var_status,
        createdAt: var_createdAt,
        lastModified: var_lastModified,
        booksCount: var_booksCount,
        booksReadCount: var_booksReadCount,
        booksUnreadCount: var_booksUnreadCount,
        booksInProgressCount: var_booksInProgressCount,
        ftsRowid: var_ftsRowid);
  }

  @protected
  ServerInfo sse_decode_server_info(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_build = sse_decode_opt_box_autoadd_build_info(deserializer);
    var var_git = sse_decode_opt_box_autoadd_git_info(deserializer);
    return ServerInfo(build: var_build, git: var_git);
  }

  @protected
  ServerProfile sse_decode_server_profile(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_displayName = sse_decode_String(deserializer);
    var var_baseUrl = sse_decode_String(deserializer);
    var var_authType = sse_decode_auth_type(deserializer);
    var var_credentialRef = sse_decode_opt_String(deserializer);
    var var_capabilities = sse_decode_list_String(deserializer);
    var var_lastSuccessfulConnection = sse_decode_opt_String(deserializer);
    return ServerProfile(
        id: var_id,
        displayName: var_displayName,
        baseUrl: var_baseUrl,
        authType: var_authType,
        credentialRef: var_credentialRef,
        capabilities: var_capabilities,
        lastSuccessfulConnection: var_lastSuccessfulConnection);
  }

  @protected
  SsePollResult sse_decode_sse_poll_result(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_stateJson = sse_decode_String(deserializer);
    var var_action = sse_decode_String(deserializer);
    var var_reconcile = sse_decode_bool(deserializer);
    var var_dirtyBooks = sse_decode_list_String(deserializer);
    var var_phase = sse_decode_String(deserializer);
    var var_reason = sse_decode_opt_String(deserializer);
    var var_booksWritten = sse_decode_i_64(deserializer);
    var var_booksDeleted = sse_decode_i_64(deserializer);
    var var_keepSocket = sse_decode_bool(deserializer);
    return SsePollResult(
        stateJson: var_stateJson,
        action: var_action,
        reconcile: var_reconcile,
        dirtyBooks: var_dirtyBooks,
        phase: var_phase,
        reason: var_reason,
        booksWritten: var_booksWritten,
        booksDeleted: var_booksDeleted,
        keepSocket: var_keepSocket);
  }

  @protected
  StorageBookDto sse_decode_storage_book_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_bookId = sse_decode_String(deserializer);
    var var_title = sse_decode_String(deserializer);
    var var_seriesTitle = sse_decode_String(deserializer);
    var var_state = sse_decode_String(deserializer);
    var var_pagesTotal = sse_decode_i_64(deserializer);
    var var_pagesDone = sse_decode_i_64(deserializer);
    var var_bytesTotal = sse_decode_i_64(deserializer);
    var var_bytesDone = sse_decode_i_64(deserializer);
    var var_onDisk = sse_decode_i_64(deserializer);
    return StorageBookDto(
        serverId: var_serverId,
        bookId: var_bookId,
        title: var_title,
        seriesTitle: var_seriesTitle,
        state: var_state,
        pagesTotal: var_pagesTotal,
        pagesDone: var_pagesDone,
        bytesTotal: var_bytesTotal,
        bytesDone: var_bytesDone,
        onDisk: var_onDisk);
  }

  @protected
  StorageDto sse_decode_storage_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_downloadBytes = sse_decode_i_64(deserializer);
    var var_downloadPageCount = sse_decode_i_64(deserializer);
    var var_bookCount = sse_decode_i_64(deserializer);
    var var_perBook = sse_decode_list_storage_book_dto(deserializer);
    var var_downloadDiskBytes = sse_decode_i_64(deserializer);
    var var_downloadDiskFiles = sse_decode_i_64(deserializer);
    var var_unownedBooks = sse_decode_i_64(deserializer);
    var var_unownedBytes = sse_decode_i_64(deserializer);
    var var_cachePageBytes = sse_decode_i_64(deserializer);
    var var_cachePrefetchBytes = sse_decode_i_64(deserializer);
    var var_cacheThumbnailBytes = sse_decode_i_64(deserializer);
    var var_cacheTotalBytes = sse_decode_i_64(deserializer);
    var var_cacheBudgetBytes = sse_decode_i_64(deserializer);
    var var_freeVolumeBytes = sse_decode_i_64(deserializer);
    return StorageDto(
        downloadBytes: var_downloadBytes,
        downloadPageCount: var_downloadPageCount,
        bookCount: var_bookCount,
        perBook: var_perBook,
        downloadDiskBytes: var_downloadDiskBytes,
        downloadDiskFiles: var_downloadDiskFiles,
        unownedBooks: var_unownedBooks,
        unownedBytes: var_unownedBytes,
        cachePageBytes: var_cachePageBytes,
        cachePrefetchBytes: var_cachePrefetchBytes,
        cacheThumbnailBytes: var_cacheThumbnailBytes,
        cacheTotalBytes: var_cacheTotalBytes,
        cacheBudgetBytes: var_cacheBudgetBytes,
        freeVolumeBytes: var_freeVolumeBytes);
  }

  @protected
  TableRows sse_decode_table_rows(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_table = sse_decode_String(deserializer);
    var var_rows = sse_decode_i_64(deserializer);
    return TableRows(table: var_table, rows: var_rows);
  }

  @protected
  ThumbnailRow sse_decode_thumbnail_row(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_variant = sse_decode_String(deserializer);
    var var_localPath = sse_decode_String(deserializer);
    var var_sizeBytes = sse_decode_i_64(deserializer);
    var var_lastAccess = sse_decode_String(deserializer);
    return ThumbnailRow(
        serverId: var_serverId,
        remoteId: var_remoteId,
        variant: var_variant,
        localPath: var_localPath,
        sizeBytes: var_sizeBytes,
        lastAccess: var_lastAccess);
  }

  @protected
  Tombstone sse_decode_tombstone(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_entityType = sse_decode_String(deserializer);
    var var_remoteId = sse_decode_String(deserializer);
    var var_deletedAt = sse_decode_String(deserializer);
    var var_cause = sse_decode_String(deserializer);
    return Tombstone(
        serverId: var_serverId,
        entityType: var_entityType,
        remoteId: var_remoteId,
        deletedAt: var_deletedAt,
        cause: var_cause);
  }

  @protected
  int sse_decode_u_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint32();
  }

  @protected
  int sse_decode_u_8(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint8();
  }

  @protected
  void sse_decode_unit(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
  }

  @protected
  UploadOutcomeDto sse_decode_upload_outcome_dto(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_serverId = sse_decode_String(deserializer);
    var var_considered = sse_decode_i_64(deserializer);
    var var_uploaded = sse_decode_i_64(deserializer);
    var var_alreadyApplied = sse_decode_i_64(deserializer);
    var var_remoteWins = sse_decode_i_64(deserializer);
    var var_gone = sse_decode_i_64(deserializer);
    var var_retried = sse_decode_i_64(deserializer);
    var var_rejected = sse_decode_i_64(deserializer);
    var var_blockedAuthentication = sse_decode_i_64(deserializer);
    var var_status = sse_decode_String(deserializer);
    var var_outbox = sse_decode_outbox_status_dto(deserializer);
    return UploadOutcomeDto(
        serverId: var_serverId,
        considered: var_considered,
        uploaded: var_uploaded,
        alreadyApplied: var_alreadyApplied,
        remoteWins: var_remoteWins,
        gone: var_gone,
        retried: var_retried,
        rejected: var_rejected,
        blockedAuthentication: var_blockedAuthentication,
        status: var_status,
        outbox: var_outbox);
  }

  @protected
  BigInt sse_decode_usize(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getBigUint64();
  }

  @protected
  void sse_encode_Map_String_String_None(
      Map<String, String> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_record_string_string(
        self.entries.map((e) => (e.key, e.value)).toList(), serializer);
  }

  @protected
  void sse_encode_String(String self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(utf8.encoder.convert(self), serializer);
  }

  @protected
  void sse_encode_api_policy_dto(ApiPolicyDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.contractVersion, serializer);
    sse_encode_String(self.snapshotVersion, serializer);
    sse_encode_String(self.minServerVersion, serializer);
  }

  @protected
  void sse_encode_auth_state_dto(AuthStateDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.state, serializer);
    sse_encode_String(self.at, serializer);
  }

  @protected
  void sse_encode_auth_type(AuthType self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_author_row(AuthorRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.name, serializer);
    sse_encode_String(self.role, serializer);
  }

  @protected
  void sse_encode_book_detail_row(
      BookDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.seriesId, serializer);
    sse_encode_opt_String(self.seriesTitle, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_opt_String(self.number, serializer);
    sse_encode_opt_box_autoadd_f_64(self.numberSort, serializer);
    sse_encode_opt_String(self.summary, serializer);
    sse_encode_opt_String(self.isbn, serializer);
    sse_encode_opt_String(self.releaseDate, serializer);
    sse_encode_opt_String(self.mediaType, serializer);
    sse_encode_opt_box_autoadd_i_64(self.pagesCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.fileSize, serializer);
    sse_encode_opt_String(self.createdAt, serializer);
    sse_encode_opt_String(self.lastModified, serializer);
    sse_encode_list_String(self.tags, serializer);
    sse_encode_list_author_row(self.authors, serializer);
    sse_encode_opt_box_autoadd_i_64(self.progressPage, serializer);
    sse_encode_bool(self.progressCompleted, serializer);
  }

  @protected
  void sse_encode_book_page_result(
      BookPageResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_book_row(self.items, serializer);
    sse_encode_i_64(self.total, serializer);
  }

  @protected
  void sse_encode_book_row(BookRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.seriesId, serializer);
    sse_encode_opt_String(self.seriesTitle, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_opt_String(self.number, serializer);
    sse_encode_opt_box_autoadd_f_64(self.numberSort, serializer);
    sse_encode_opt_box_autoadd_i_64(self.fileSize, serializer);
    sse_encode_opt_String(self.mediaType, serializer);
    sse_encode_opt_box_autoadd_i_64(self.pagesCount, serializer);
    sse_encode_opt_String(self.createdAt, serializer);
    sse_encode_opt_String(self.lastModified, serializer);
    sse_encode_opt_box_autoadd_i_64(self.progressPage, serializer);
    sse_encode_bool(self.progressCompleted, serializer);
    sse_encode_opt_box_autoadd_i_64(self.ftsRowid, serializer);
  }

  @protected
  void sse_encode_bool(bool self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint8(self ? 1 : 0);
  }

  @protected
  void sse_encode_bootstrap_summary(
      BootstrapSummary self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_usize(self.syncedSeries, serializer);
    sse_encode_i_64(self.totalElements, serializer);
    sse_encode_bool(self.hasMorePages, serializer);
  }

  @protected
  void sse_encode_box_autoadd_book_detail_row(
      BookDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_book_detail_row(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_bool(bool self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_bool(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_build_info(
      BuildInfo self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_build_info(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_collection_detail_row(
      CollectionDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_collection_detail_row(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_device_profile_dto(
      DeviceProfileDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_device_profile_dto(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_download_pump_dto(
      DownloadPumpDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_download_pump_dto(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_f_64(double self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_f_64(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_git_commit(
      GitCommit self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_git_commit(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_git_info(GitInfo self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_git_info(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_i_64(
      PlatformInt64 self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_library_count_row(
      LibraryCountRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_library_count_row(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_reader_settings_dto(
      ReaderSettingsDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_reader_settings_dto(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_readlist_detail_row(
      ReadlistDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_readlist_detail_row(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_series_detail_row(
      SeriesDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_series_detail_row(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_server_profile(
      ServerProfile self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_server_profile(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_sse_poll_result(
      SsePollResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_sse_poll_result(self, serializer);
  }

  @protected
  void sse_encode_build_info(BuildInfo self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.artifact, serializer);
    sse_encode_opt_String(self.name, serializer);
    sse_encode_opt_String(self.time, serializer);
    sse_encode_opt_String(self.version, serializer);
    sse_encode_opt_String(self.group, serializer);
  }

  @protected
  void sse_encode_cache_cleanup_dto(
      CacheCleanupDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.ghostRows, serializer);
    sse_encode_i_64(self.orphanFiles, serializer);
    sse_encode_i_64(self.staleParts, serializer);
    sse_encode_i_64(self.corrupt, serializer);
    sse_encode_i_64(self.kindRepaired, serializer);
    sse_encode_i_64(self.evicted, serializer);
    sse_encode_i_64(self.freedBytes, serializer);
    sse_encode_i_64(self.bytesAfter, serializer);
  }

  @protected
  void sse_encode_cache_stats_dto(
      CacheStatsDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.pageBytes, serializer);
    sse_encode_i_64(self.prefetchBytes, serializer);
    sse_encode_i_64(self.downloadBytes, serializer);
    sse_encode_i_64(self.poolBudgetBytes, serializer);
    sse_encode_i_64(self.memoryBytes, serializer);
    sse_encode_i_64(self.memoryPeakBytes, serializer);
    sse_encode_i_64(self.memoryEntries, serializer);
    sse_encode_i_64(self.memoryHits, serializer);
    sse_encode_i_64(self.memoryMisses, serializer);
    sse_encode_i_64(self.memoryEvictions, serializer);
    sse_encode_i_64(self.memoryRefused, serializer);
    sse_encode_i_64(self.diskBytes, serializer);
    sse_encode_i_64(self.ledgerBytes, serializer);
    sse_encode_i_64(self.openReaders, serializer);
  }

  @protected
  void sse_encode_collection_detail_row(
      CollectionDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_bool(self.ordered, serializer);
    sse_encode_bool(self.filtered, serializer);
    sse_encode_opt_String(self.createdDate, serializer);
    sse_encode_opt_String(self.lastModifiedDate, serializer);
    sse_encode_series_page_result(self.members, serializer);
  }

  @protected
  void sse_encode_collection_page_result(
      CollectionPageResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_collection_row(self.items, serializer);
    sse_encode_i_64(self.total, serializer);
  }

  @protected
  void sse_encode_collection_ref(CollectionRef self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.name, serializer);
  }

  @protected
  void sse_encode_collection_row(CollectionRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_bool(self.ordered, serializer);
    sse_encode_bool(self.filtered, serializer);
    sse_encode_opt_String(self.createdDate, serializer);
    sse_encode_opt_String(self.lastModifiedDate, serializer);
  }

  @protected
  void sse_encode_connection_result(
      ConnectionResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_server_info(self.serverInfo, serializer);
    sse_encode_opt_String(self.serverVersion, serializer);
    sse_encode_list_library(self.libraries, serializer);
    sse_encode_list_String(self.capabilities, serializer);
  }

  @protected
  void sse_encode_continue_reading_row(
      ContinueReadingRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.bookId, serializer);
    sse_encode_String(self.bookTitle, serializer);
    sse_encode_opt_String(self.number, serializer);
    sse_encode_String(self.seriesId, serializer);
    sse_encode_String(self.seriesName, serializer);
    sse_encode_opt_box_autoadd_i_64(self.page, serializer);
    sse_encode_opt_box_autoadd_i_64(self.totalPages, serializer);
    sse_encode_opt_box_autoadd_i_64(self.progressPct, serializer);
    sse_encode_opt_String(self.localUpdatedAt, serializer);
  }

  @protected
  void sse_encode_core_error(CoreError self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_error_code(self.code, serializer);
    sse_encode_String(self.message, serializer);
    sse_encode_bool(self.retryable, serializer);
    sse_encode_bool(self.needsUser, serializer);
  }

  @protected
  void sse_encode_db_health(DbHealth self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.schemaVersion, serializer);
    sse_encode_String(self.integrity, serializer);
    sse_encode_String(self.journalMode, serializer);
    sse_encode_i_64(self.pageSize, serializer);
    sse_encode_i_64(self.pageCount, serializer);
    sse_encode_i_64(self.freelistCount, serializer);
    sse_encode_i_64(self.busyTimeoutMs, serializer);
    sse_encode_bool(self.foreignKeysOn, serializer);
    sse_encode_i_64(self.fileBytes, serializer);
    sse_encode_list_table_rows(self.tables, serializer);
  }

  @protected
  void sse_encode_device_profile_dto(
      DeviceProfileDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.deviceMemoryBytes, serializer);
    sse_encode_i_64(self.cacheBudgetBytes, serializer);
    sse_encode_i_64(self.avgPageBytesHint, serializer);
    sse_encode_i_64(self.decodedPageBytes, serializer);
    sse_encode_String(self.network, serializer);
    sse_encode_bool(self.stable, serializer);
  }

  @protected
  void sse_encode_diagnostics_dto(
      DiagnosticsDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_db_health(self.db, serializer);
    sse_encode_auth_state_dto(self.auth, serializer);
    sse_encode_i_64(self.outboxQueuedRows, serializer);
    sse_encode_list_entity_sync_state(self.sync_, serializer);
    sse_encode_outbox_status_dto(self.outbox, serializer);
    sse_encode_cache_stats_dto(self.cache, serializer);
    sse_encode_storage_dto(self.storage, serializer);
    sse_encode_list_queue_state_count_dto(self.queue, serializer);
    sse_encode_api_policy_dto(self.policy, serializer);
    sse_encode_log_stats(self.log, serializer);
  }

  @protected
  void sse_encode_download_book_dto(
      DownloadBookDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.bookId, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.seriesTitle, serializer);
    sse_encode_String(self.state, serializer);
    sse_encode_i_64(self.pagesTotal, serializer);
    sse_encode_i_64(self.pagesDone, serializer);
    sse_encode_i_64(self.bytesTotal, serializer);
    sse_encode_i_64(self.bytesDone, serializer);
    sse_encode_i_64(self.position, serializer);
    sse_encode_String(self.lastError, serializer);
    sse_encode_String(self.nextRetryAt, serializer);
    sse_encode_String(self.remoteLastModified, serializer);
    sse_encode_bool(self.allowCellular, serializer);
    sse_encode_bool(self.stale, serializer);
  }

  @protected
  void sse_encode_download_delete_dto(
      DownloadDeleteDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.books, serializer);
    sse_encode_i_64(self.files, serializer);
    sse_encode_i_64(self.freedBytes, serializer);
  }

  @protected
  void sse_encode_download_pump_dto(
      DownloadPumpDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.book, serializer);
    sse_encode_String(self.state, serializer);
    sse_encode_i_64(self.served, serializer);
    sse_encode_i_64(self.failedPages, serializer);
    sse_encode_i_64(self.bytesWritten, serializer);
    sse_encode_i_64(self.pagesDone, serializer);
    sse_encode_i_64(self.pagesTotal, serializer);
    sse_encode_String(self.stopReason, serializer);
    sse_encode_i_64(self.nextInMs, serializer);
    sse_encode_i_64(self.pumpMs, serializer);
    sse_encode_String(self.lastError, serializer);
    sse_encode_i_64(self.repairs, serializer);
    sse_encode_i_64(self.partsSwept, serializer);
    sse_encode_i_64(self.adopted, serializer);
    sse_encode_i_64(self.ghostRows, serializer);
    sse_encode_bool(self.queueActive, serializer);
  }

  @protected
  void sse_encode_download_sweep_dto(
      DownloadSweepDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.books, serializer);
    sse_encode_i_64(self.staleParts, serializer);
    sse_encode_i_64(self.ghostRows, serializer);
    sse_encode_i_64(self.corrupt, serializer);
    sse_encode_i_64(self.sizeMismatch, serializer);
    sse_encode_i_64(self.adoptedFiles, serializer);
    sse_encode_i_64(self.countersRepaired, serializer);
    sse_encode_i_64(self.manifestsRewritten, serializer);
    sse_encode_i_64(self.pagesRemoved, serializer);
    sse_encode_i_64(self.unownedBooks, serializer);
    sse_encode_i_64(self.unownedBytes, serializer);
    sse_encode_i_64(self.freedBytes, serializer);
  }

  @protected
  void sse_encode_entity_sync_state(
      EntitySyncState self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.entityType, serializer);
    sse_encode_opt_String(self.lastSyncAt, serializer);
    sse_encode_opt_String(self.syncCursor, serializer);
    sse_encode_String(self.syncStatus, serializer);
    sse_encode_opt_String(self.lastError, serializer);
    sse_encode_opt_String(self.lastFullSync, serializer);
    sse_encode_opt_String(self.lastSuccessfulSync, serializer);
  }

  @protected
  void sse_encode_error_code(ErrorCode self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_f_64(double self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putFloat64(self);
  }

  @protected
  void sse_encode_filter_options(FilterOptions self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_String(self.tags, serializer);
    sse_encode_list_String(self.genres, serializer);
    sse_encode_list_String(self.statuses, serializer);
  }

  @protected
  void sse_encode_full_sync_summary(
      FullSyncSummary self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_usize(self.libraries, serializer);
    sse_encode_usize(self.series, serializer);
    sse_encode_usize(self.books, serializer);
    sse_encode_usize(self.collections, serializer);
    sse_encode_usize(self.readlists, serializer);
    sse_encode_usize(self.readProgress, serializer);
    sse_encode_u_32(self.seriesPages, serializer);
    sse_encode_u_32(self.bookPages, serializer);
    sse_encode_list_String(self.skippedSteps, serializer);
    sse_encode_list_String(self.resumedSteps, serializer);
  }

  @protected
  void sse_encode_git_commit(GitCommit self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.id, serializer);
    sse_encode_opt_String(self.time, serializer);
  }

  @protected
  void sse_encode_git_info(GitInfo self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.branch, serializer);
    sse_encode_opt_box_autoadd_git_commit(self.commit, serializer);
  }

  @protected
  void sse_encode_i_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putInt32(self);
  }

  @protected
  void sse_encode_i_64(PlatformInt64 self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putPlatformInt64(self);
  }

  @protected
  void sse_encode_library(Library self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_String(self.root, serializer);
    sse_encode_opt_box_autoadd_bool(self.unavailable, serializer);
  }

  @protected
  void sse_encode_library_count_row(
      LibraryCountRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_opt_String(self.root, serializer);
    sse_encode_bool(self.unavailable, serializer);
    sse_encode_i_64(self.seriesCount, serializer);
    sse_encode_i_64(self.bookCount, serializer);
    sse_encode_i_64(self.readCount, serializer);
  }

  @protected
  void sse_encode_list_String(List<String> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_String(item, serializer);
    }
  }

  @protected
  void sse_encode_list_author_row(
      List<AuthorRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_author_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_book_row(List<BookRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_book_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_collection_ref(
      List<CollectionRef> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_collection_ref(item, serializer);
    }
  }

  @protected
  void sse_encode_list_collection_row(
      List<CollectionRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_collection_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_continue_reading_row(
      List<ContinueReadingRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_continue_reading_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_download_book_dto(
      List<DownloadBookDto> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_download_book_dto(item, serializer);
    }
  }

  @protected
  void sse_encode_list_entity_sync_state(
      List<EntitySyncState> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_entity_sync_state(item, serializer);
    }
  }

  @protected
  void sse_encode_list_library(List<Library> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_library(item, serializer);
    }
  }

  @protected
  void sse_encode_list_library_count_row(
      List<LibraryCountRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_library_count_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_list_prim_u_32_strict(
      List<Uint32List> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_list_prim_u_32_strict(item, serializer);
    }
  }

  @protected
  void sse_encode_list_log_record(
      List<LogRecord> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_log_record(item, serializer);
    }
  }

  @protected
  void sse_encode_list_outbox_entry_dto(
      List<OutboxEntryDto> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_outbox_entry_dto(item, serializer);
    }
  }

  @protected
  void sse_encode_list_prim_u_32_strict(
      Uint32List self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    serializer.buffer.putUint32List(self);
  }

  @protected
  void sse_encode_list_prim_u_8_strict(
      Uint8List self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    serializer.buffer.putUint8List(self);
  }

  @protected
  void sse_encode_list_queue_state_count_dto(
      List<QueueStateCountDto> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_queue_state_count_dto(item, serializer);
    }
  }

  @protected
  void sse_encode_list_readlist_row(
      List<ReadlistRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_readlist_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_record_string_string(
      List<(String, String)> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_record_string_string(item, serializer);
    }
  }

  @protected
  void sse_encode_list_series_row(
      List<SeriesRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_series_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_server_profile(
      List<ServerProfile> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_server_profile(item, serializer);
    }
  }

  @protected
  void sse_encode_list_storage_book_dto(
      List<StorageBookDto> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_storage_book_dto(item, serializer);
    }
  }

  @protected
  void sse_encode_list_table_rows(
      List<TableRows> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_table_rows(item, serializer);
    }
  }

  @protected
  void sse_encode_list_thumbnail_row(
      List<ThumbnailRow> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_thumbnail_row(item, serializer);
    }
  }

  @protected
  void sse_encode_list_tombstone(
      List<Tombstone> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_tombstone(item, serializer);
    }
  }

  @protected
  void sse_encode_log_record(LogRecord self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.level, serializer);
    sse_encode_String(self.target, serializer);
    sse_encode_String(self.message, serializer);
    sse_encode_String(self.at, serializer);
  }

  @protected
  void sse_encode_log_stats(LogStats self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_bool(self.installed, serializer);
    sse_encode_i_64(self.capacity, serializer);
    sse_encode_i_64(self.retained, serializer);
    sse_encode_i_64(self.dropped, serializer);
    sse_encode_i_64(self.errors, serializer);
    sse_encode_i_64(self.warnings, serializer);
    sse_encode_i_64(self.info, serializer);
    sse_encode_i_64(self.debug, serializer);
    sse_encode_i_64(self.trace, serializer);
    sse_encode_opt_String(self.lastError, serializer);
    sse_encode_String(self.maxLevel, serializer);
  }

  @protected
  void sse_encode_opt_String(String? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_String(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_book_detail_row(
      BookDetailRow? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_book_detail_row(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_bool(bool? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_bool(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_build_info(
      BuildInfo? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_build_info(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_collection_detail_row(
      CollectionDetailRow? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_collection_detail_row(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_download_pump_dto(
      DownloadPumpDto? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_download_pump_dto(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_f_64(double? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_f_64(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_git_commit(
      GitCommit? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_git_commit(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_git_info(
      GitInfo? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_git_info(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_i_64(
      PlatformInt64? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_i_64(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_library_count_row(
      LibraryCountRow? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_library_count_row(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_readlist_detail_row(
      ReadlistDetailRow? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_readlist_detail_row(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_series_detail_row(
      SeriesDetailRow? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_series_detail_row(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_server_profile(
      ServerProfile? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_server_profile(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_sse_poll_result(
      SsePollResult? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_sse_poll_result(self, serializer);
    }
  }

  @protected
  void sse_encode_outbox_entry_dto(
      OutboxEntryDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.entityId, serializer);
    sse_encode_String(self.mutationType, serializer);
    sse_encode_i_64(self.retryCount, serializer);
    sse_encode_opt_String(self.lastError, serializer);
    sse_encode_String(self.state, serializer);
    sse_encode_opt_String(self.nextRetryAt, serializer);
    sse_encode_String(self.createdAt, serializer);
  }

  @protected
  void sse_encode_outbox_status_dto(
      OutboxStatusDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_i_64(self.pending, serializer);
    sse_encode_i_64(self.waiting, serializer);
    sse_encode_i_64(self.failed, serializer);
    sse_encode_i_64(self.total, serializer);
    sse_encode_list_outbox_entry_dto(self.failedEntries, serializer);
  }

  @protected
  void sse_encode_queue_state_count_dto(
      QueueStateCountDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.state, serializer);
    sse_encode_i_64(self.books, serializer);
    sse_encode_i_64(self.pagesDone, serializer);
    sse_encode_i_64(self.pagesTotal, serializer);
    sse_encode_i_64(self.bytesDone, serializer);
    sse_encode_i_64(self.bytesTotal, serializer);
  }

  @protected
  void sse_encode_read_target_row(
      ReadTargetRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_book_row(self.book, serializer);
    sse_encode_String(self.intent, serializer);
    sse_encode_i_64(self.position, serializer);
    sse_encode_opt_box_autoadd_i_64(self.bookCount, serializer);
    sse_encode_bool(self.complete, serializer);
  }

  @protected
  void sse_encode_reader_book_dto(
      ReaderBookDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.bookId, serializer);
    sse_encode_i_64(self.pageCount, serializer);
    sse_encode_bool(self.paged, serializer);
    sse_encode_bool(self.reflowable, serializer);
    sse_encode_opt_String(self.fallback, serializer);
    sse_encode_bool(self.fromMirror, serializer);
    sse_encode_i_64(self.startPage, serializer);
    sse_encode_opt_box_autoadd_f_64(self.startPageOffsetRatio, serializer);
    sse_encode_reader_layout_dto(self.layout, serializer);
  }

  @protected
  void sse_encode_reader_layout_dto(
      ReaderLayoutDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_list_prim_u_32_strict(self.spreads, serializer);
    sse_encode_i_64(self.spread, serializer);
    sse_encode_i_64(self.page, serializer);
    sse_encode_String(self.axis, serializer);
    sse_encode_bool(self.reversed, serializer);
    sse_encode_String(self.advanceSwipe, serializer);
    sse_encode_String(self.retreatSwipe, serializer);
    sse_encode_String(self.tapNext, serializer);
    sse_encode_String(self.tapPrev, serializer);
    sse_encode_String(self.mode, serializer);
    sse_encode_String(self.direction, serializer);
    sse_encode_i_64(self.pageGap, serializer);
    sse_encode_String(self.background, serializer);
  }

  @protected
  void sse_encode_reader_settings_dto(
      ReaderSettingsDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.mode, serializer);
    sse_encode_String(self.direction, serializer);
    sse_encode_bool(self.firstPageSingle, serializer);
    sse_encode_i_64(self.pageGap, serializer);
    sse_encode_String(self.background, serializer);
    sse_encode_bool(self.keepScreenAwake, serializer);
    sse_encode_opt_box_autoadd_f_64(self.brightness, serializer);
    sse_encode_bool(self.restorePosition, serializer);
    sse_encode_i_64(self.prefetchForward, serializer);
    sse_encode_i_64(self.prefetchBack, serializer);
    sse_encode_i_64(self.prefetchCap, serializer);
    sse_encode_bool(self.volumeKeysEnabled, serializer);
  }

  @protected
  void sse_encode_reader_turn_dto(
      ReaderTurnDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.page, serializer);
    sse_encode_i_64(self.spread, serializer);
    sse_encode_bool(self.uploadNow, serializer);
  }

  @protected
  void sse_encode_reader_window_dto(
      ReaderWindowDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.forward, serializer);
    sse_encode_i_64(self.back, serializer);
    sse_encode_i_64(self.cap, serializer);
    sse_encode_i_64(self.memoryBudgetBytes, serializer);
    sse_encode_i_64(self.inFlight, serializer);
    sse_encode_i_64(self.decodeSlots, serializer);
    sse_encode_i_64(self.avgPageBytes, serializer);
    sse_encode_i_64(self.pagesPerSpread, serializer);
    sse_encode_i_64(self.poolBudgetBytes, serializer);
    sse_encode_i_64(self.sweptFreedBytes, serializer);
    sse_encode_i_64(self.sweptCorrupt, serializer);
  }

  @protected
  void sse_encode_readlist_detail_row(
      ReadlistDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_opt_String(self.summary, serializer);
    sse_encode_bool(self.ordered, serializer);
    sse_encode_bool(self.filtered, serializer);
    sse_encode_opt_String(self.createdDate, serializer);
    sse_encode_opt_String(self.lastModifiedDate, serializer);
    sse_encode_book_page_result(self.books, serializer);
  }

  @protected
  void sse_encode_readlist_page_result(
      ReadlistPageResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_readlist_row(self.items, serializer);
    sse_encode_i_64(self.total, serializer);
  }

  @protected
  void sse_encode_readlist_row(ReadlistRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_opt_String(self.summary, serializer);
    sse_encode_bool(self.ordered, serializer);
    sse_encode_bool(self.filtered, serializer);
    sse_encode_opt_String(self.createdDate, serializer);
    sse_encode_opt_String(self.lastModifiedDate, serializer);
  }

  @protected
  void sse_encode_reconcile_summary(
      ReconcileSummary self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.trigger, serializer);
    sse_encode_usize(self.seriesUpserted, serializer);
    sse_encode_usize(self.seriesAdded, serializer);
    sse_encode_usize(self.seriesChanged, serializer);
    sse_encode_usize(self.seriesRemoved, serializer);
    sse_encode_usize(self.booksUpserted, serializer);
    sse_encode_usize(self.booksAdded, serializer);
    sse_encode_usize(self.booksChanged, serializer);
    sse_encode_usize(self.booksRemoved, serializer);
    sse_encode_usize(self.collectionsUpserted, serializer);
    sse_encode_usize(self.collectionsAdded, serializer);
    sse_encode_usize(self.collectionsChanged, serializer);
    sse_encode_usize(self.collectionsRemoved, serializer);
    sse_encode_usize(self.readlistsUpserted, serializer);
    sse_encode_usize(self.readlistsAdded, serializer);
    sse_encode_usize(self.readlistsChanged, serializer);
    sse_encode_usize(self.readlistsRemoved, serializer);
    sse_encode_usize(self.librariesUpserted, serializer);
    sse_encode_usize(self.librariesRemoved, serializer);
    sse_encode_usize(self.readProgress, serializer);
    sse_encode_u_32(self.pagesSwept, serializer);
    sse_encode_list_String(self.orphanedCovers, serializer);
    sse_encode_bool(self.clean, serializer);
  }

  @protected
  void sse_encode_record_string_string(
      (String, String) self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.$1, serializer);
    sse_encode_String(self.$2, serializer);
  }

  @protected
  void sse_encode_series_detail_row(
      SeriesDetailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.libraryId, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_opt_String(self.sortName, serializer);
    sse_encode_opt_String(self.status, serializer);
    sse_encode_opt_String(self.createdAt, serializer);
    sse_encode_opt_String(self.lastModified, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksReadCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksUnreadCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksInProgressCount, serializer);
    sse_encode_opt_String(self.summary, serializer);
    sse_encode_opt_String(self.publisher, serializer);
    sse_encode_opt_String(self.readingDirection, serializer);
    sse_encode_opt_String(self.language, serializer);
    sse_encode_opt_String(self.ageRating, serializer);
    sse_encode_opt_box_autoadd_i_64(self.totalBookCount, serializer);
    sse_encode_list_String(self.genres, serializer);
    sse_encode_list_String(self.tags, serializer);
    sse_encode_list_author_row(self.authors, serializer);
    sse_encode_list_collection_ref(self.collections, serializer);
  }

  @protected
  void sse_encode_series_page_result(
      SeriesPageResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_series_row(self.items, serializer);
    sse_encode_i_64(self.total, serializer);
  }

  @protected
  void sse_encode_series_row(SeriesRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.libraryId, serializer);
    sse_encode_String(self.name, serializer);
    sse_encode_opt_String(self.sortName, serializer);
    sse_encode_opt_String(self.status, serializer);
    sse_encode_opt_String(self.createdAt, serializer);
    sse_encode_opt_String(self.lastModified, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksReadCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksUnreadCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.booksInProgressCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.ftsRowid, serializer);
  }

  @protected
  void sse_encode_server_info(ServerInfo self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_box_autoadd_build_info(self.build, serializer);
    sse_encode_opt_box_autoadd_git_info(self.git, serializer);
  }

  @protected
  void sse_encode_server_profile(ServerProfile self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.displayName, serializer);
    sse_encode_String(self.baseUrl, serializer);
    sse_encode_auth_type(self.authType, serializer);
    sse_encode_opt_String(self.credentialRef, serializer);
    sse_encode_list_String(self.capabilities, serializer);
    sse_encode_opt_String(self.lastSuccessfulConnection, serializer);
  }

  @protected
  void sse_encode_sse_poll_result(
      SsePollResult self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.stateJson, serializer);
    sse_encode_String(self.action, serializer);
    sse_encode_bool(self.reconcile, serializer);
    sse_encode_list_String(self.dirtyBooks, serializer);
    sse_encode_String(self.phase, serializer);
    sse_encode_opt_String(self.reason, serializer);
    sse_encode_i_64(self.booksWritten, serializer);
    sse_encode_i_64(self.booksDeleted, serializer);
    sse_encode_bool(self.keepSocket, serializer);
  }

  @protected
  void sse_encode_storage_book_dto(
      StorageBookDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.bookId, serializer);
    sse_encode_String(self.title, serializer);
    sse_encode_String(self.seriesTitle, serializer);
    sse_encode_String(self.state, serializer);
    sse_encode_i_64(self.pagesTotal, serializer);
    sse_encode_i_64(self.pagesDone, serializer);
    sse_encode_i_64(self.bytesTotal, serializer);
    sse_encode_i_64(self.bytesDone, serializer);
    sse_encode_i_64(self.onDisk, serializer);
  }

  @protected
  void sse_encode_storage_dto(StorageDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self.downloadBytes, serializer);
    sse_encode_i_64(self.downloadPageCount, serializer);
    sse_encode_i_64(self.bookCount, serializer);
    sse_encode_list_storage_book_dto(self.perBook, serializer);
    sse_encode_i_64(self.downloadDiskBytes, serializer);
    sse_encode_i_64(self.downloadDiskFiles, serializer);
    sse_encode_i_64(self.unownedBooks, serializer);
    sse_encode_i_64(self.unownedBytes, serializer);
    sse_encode_i_64(self.cachePageBytes, serializer);
    sse_encode_i_64(self.cachePrefetchBytes, serializer);
    sse_encode_i_64(self.cacheThumbnailBytes, serializer);
    sse_encode_i_64(self.cacheTotalBytes, serializer);
    sse_encode_i_64(self.cacheBudgetBytes, serializer);
    sse_encode_i_64(self.freeVolumeBytes, serializer);
  }

  @protected
  void sse_encode_table_rows(TableRows self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.table, serializer);
    sse_encode_i_64(self.rows, serializer);
  }

  @protected
  void sse_encode_thumbnail_row(ThumbnailRow self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.variant, serializer);
    sse_encode_String(self.localPath, serializer);
    sse_encode_i_64(self.sizeBytes, serializer);
    sse_encode_String(self.lastAccess, serializer);
  }

  @protected
  void sse_encode_tombstone(Tombstone self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_String(self.entityType, serializer);
    sse_encode_String(self.remoteId, serializer);
    sse_encode_String(self.deletedAt, serializer);
    sse_encode_String(self.cause, serializer);
  }

  @protected
  void sse_encode_u_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint32(self);
  }

  @protected
  void sse_encode_u_8(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint8(self);
  }

  @protected
  void sse_encode_unit(void self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
  }

  @protected
  void sse_encode_upload_outcome_dto(
      UploadOutcomeDto self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.serverId, serializer);
    sse_encode_i_64(self.considered, serializer);
    sse_encode_i_64(self.uploaded, serializer);
    sse_encode_i_64(self.alreadyApplied, serializer);
    sse_encode_i_64(self.remoteWins, serializer);
    sse_encode_i_64(self.gone, serializer);
    sse_encode_i_64(self.retried, serializer);
    sse_encode_i_64(self.rejected, serializer);
    sse_encode_i_64(self.blockedAuthentication, serializer);
    sse_encode_String(self.status, serializer);
    sse_encode_outbox_status_dto(self.outbox, serializer);
  }

  @protected
  void sse_encode_usize(BigInt self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putBigUint64(self);
  }
}
